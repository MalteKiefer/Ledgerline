...
/**
 * Zero-knowledge to-do lists: everything lives in the encrypted "todos"
 * manifest (lists + tasks), exactly like notes. A task carries a title,
 * description, URL, priority, a "marked" flag, tags and an optional due
 * date/time. The server only ever stores ciphertext.
 */
Alpine.data('vaultTodos', (labels = {}) => ({
    state: 'boot', // boot | locked | unconfigured | ready | error
    manifest: { v: 1, lists: [], tasks: [] },
    version: 0,
    ...
    // Editor modal
    editorOpen: false,
    editing: null, // working copy of the task being added/edited
    tagsValue: '',
    ...
    async load() {
        try {
            const { data, version } = await Vault.loadManifest('todos');
            this.manifest = data.tasks ? data : { v: 1, lists: [], tasks: [] };
            this.version = version;
            this.state = 'ready';
    ...
    async persist() {
        try {
            this.version = await Vault.saveManifest('todos', this.manifest, this.version);
            this.error = '';
            window.dispatchEvent(new CustomEvent('todos-changed'));
        } catch (e) {
            if (e.stale) { await this.load(); this.error = labels.stale; }
    ...
    /* ---- Lists ---- */
    get lists() {
        return [...this.manifest.lists].sort((a, b) => (a.name || '').localeCompare(b.name || ''));
    },
    listName(id) { return (this.manifest.lists.find((l) => l.id === id) || {}).name || ''; },

    async addList() {
        const name = this.newListName.trim();
        if (! name) return;
        const now = new Date().toISOString();
        this.manifest.lists.push({ id: crypto.randomUUID(), name, created: now, updated: now });
        this.newListName = '';
        await this.persist();
    },
    async renameList(l) {
        ...
        l.name = name; l.updated = new Date().toISOString();
        await this.persist();
    },
    async deleteList(l) {
        if (! confirm(labels.deleteListConfirm)) return;
        // Orphan the list's tasks (keep them under "All") rather than lose them.
        this.manifest.tasks.forEach((t) => { if (t.listId === l.id) t.listId = null; });
        this.manifest.lists = this.manifest.lists.filter((x) => x.id !== l.id);
    ...
    get allTags() {
        const set = new Set();
        for (const t of this.manifest.tasks) for (const g of t.tags ?? []) set.add(g);
        return [...set].sort((a, b) => a.localeCompare(b));
    },
    get trashCount() { return this.manifest.tasks.filter((t) => t.trashed).length; },

    get tasks() {
        const q = this.query.trim().toLowerCase();
        let list = this.manifest.tasks.filter((t) => this.view === 'trash' ? t.trashed : ! t.trashed);
    ...
    async emptyTrash() {
        ...
        this.manifest.tasks = this.manifest.tasks.filter((t) => ! t.trashed);
        await this.persist();
    },

    /* ---- Display helpers ---- */
    priorityClass(p) {
        return p === 'high' ? 'bg-red-500' : (p === 'low' ? 'bg-gray-300' : 'bg-amber-400');
    },
    dueLabel(t) {
        if (! t.due) return '';
        try { return new Date(t.due).toLocaleString(); } catch (e) { return t.due; }
    },
    isOverdue(t) {
        return t.due && ! t.done && new Date(t.due).getTime() < Date.now();
    },
}));
