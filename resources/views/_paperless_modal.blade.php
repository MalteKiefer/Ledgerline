    ...
    <div x-show="$store.paperless.open" x-cloak class="fixed inset-0 z-[1100] flex items-center justify-center p-4" role="dialog" aria-modal="true"
        ...
        <div class="relative flex max-h-[92vh] w-full max-w-lg flex-col rounded-2xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] shadow-xl">
            ...
            <div class="min-h-0 flex-1 space-y-4 overflow-auto p-5" x-data="{ s: $store.paperless }">
                <p class="flex items-center gap-2 truncate text-xs text-gray-500 dark:text-gray-400">
                ...
                <div x-data="{ show: false }" @click.outside="show = false">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('paperless.correspondent') }}</label>
                    <div class="relative mt-1">
                        ...
                        <div x-show="show" x-cloak class="absolute z-30 mt-1 max-h-48 w-full overflow-auto rounded-xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] py-1 text-sm shadow-lg">
                            ...
                            <p x-show="! s.filteredCorrespondents.length && ! s.canCreate(s.correspondents, s.corrQuery)" class="px-3 py-1.5 text-gray-400 dark:text-gray-500">{{ __('paperless.none') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div x-data="{ show: false }" @click.outside="show = false">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('paperless.document_type') }}</label>
                    <div class="relative mt-1">
                        ...
                        <div x-show="show" x-cloak class="absolute z-30 mt-1 max-h-48 w-full overflow-auto rounded-xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] py-1 text-sm shadow-lg">
                            ...
                            <p x-show="! s.filteredDocumentTypes.length && ! s.canCreate(s.documentTypes, s.typeQuery)" class="px-3 py-1.5 text-gray-400 dark:text-gray-500">{{ __('paperless.none') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div x-data="{ show: false }" @click.outside="show = false">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('paperless.tags') }}</label>
                    ...
                    <div class="relative mt-1">
                        ...
                        <div x-show="show" x-cloak class="absolute bottom-full z-30 mb-1 max-h-48 w-full overflow-auto rounded-xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] py-1 text-sm shadow-lg">
                            ...
                            <p x-show="! s.filteredTags.length && ! s.canCreate(s.tags, s.tagQuery)" class="px-3 py-1.5 text-gray-400 dark:text-gray-500">{{ __('paperless.none') }}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-5 py-3 dark:border-gray-800">
                ...
                <x-button variant="primary" @click="$store.paperless.submit()" ::disabled="$store.paperless.submitting || $store.paperless.preparing || ! $store.paperless.file">
                    <span x-show="! $store.paperless.submitting">{{ __('paperless.send') }}</span>
                    <span x-show="$store.paperless.submitting" x-cloak>{{ __('paperless.sending') }}</span>
                </x-button>
            </div>
        </div>
    </div>
