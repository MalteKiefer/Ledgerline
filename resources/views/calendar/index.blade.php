<x-layouts.app :title="__('calendar.ui.heading')">
    ...
    <div x-data="calendarPage(@js($cfg))" x-init="init()" class="flex flex-col gap-4 md:flex-row">
        ...
        <div class="min-w-0 flex-1">
            ...
            <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-1">
                    <button @click="step(-1)" class="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md border border-gray-300 dark:border-gray-700 px-2 py-1.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800" title="{{ __('calendar.ui.prev') }}"><x-icon name="chevron-left" class="h-4 w-4" /></button>
                    <button @click="today()" class="inline-flex min-h-11 items-center justify-center rounded-md border border-gray-300 dark:border-gray-700 px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">{{ __('calendar.ui.today') }}</button>
                    <button @click="step(1)" class="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md border border-gray-300 dark:border-gray-700 px-2 py-1.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800" title="{{ __('calendar.ui.next') }}"><x-icon name="chevron-right" class="h-4 w-4" /></button>
                    <h2 class="ml-2 text-base font-semibold text-gray-900 dark:text-gray-100" x-text="title"></h2>
                </div>
                <div class="inline-flex flex-wrap rounded-md border border-gray-300 dark:border-gray-700 text-sm">
                    @foreach (['month', 'week', 'day', 'agenda'] as $v)
                        <button @click="setView('{{ $v }}')" :class="view==='{{ $v }}'?'bg-gray-900 dark:bg-gray-100 dark:text-gray-900 text-white':'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800'"
                            ...
                            <div @click="openNew(d)" :data-date="ymd(d)"
                                ...
                                <div class="mt-0.5 space-y-0.5">
                                    ...
                                    <template x-if="eventsOn(d).length>3">
                                        <div class="px-1 text-[10px] text-gray-500 dark:text-gray-400" x-text="'+'+(eventsOn(d).length-3)"></div>
                                    </template>
                                </div>
                            </div>
                        </template>
            ...
            <div x-show="view==='week'" class="mt-4 overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
                <div class="overflow-x-auto">
                    <div class="min-w-[48rem]">
                        {{-- Day headers --}}
                        <div class="grid border-b border-gray-200 dark:border-gray-800" style="grid-template-columns:3.5rem repeat(7,minmax(0,1fr))">
                            <div></div>
                            <template x-for="(d,i) in weekDays()" :key="'wh'+i">
                                <div class="border-l border-gray-100 dark:border-gray-800 py-1.5 text-center text-xs font-medium"
                                    :class="isToday(d)?'text-gray-900 dark:text-gray-100':'text-gray-500 dark:text-gray-400'" x-text="fmtDay(d)"></div>
                            </template>
                        </div>
                        {{-- All-day strip --}}
                            ...
                            <div class="grid" style="grid-template-columns:3.5rem repeat(7,minmax(0,1fr))">
                                ...
                                <div class="relative" :style="`height:${24*hourPx}px`">
                                    <template x-for="h in hours()" :key="'wg'+h">
                                        <div class="absolute right-1 -translate-y-1/2 text-[10px] text-gray-400 dark:text-gray-500" :style="`top:${h*hourPx}px`" x-text="h===0?'':fmtHour(h)"></div>
                                    </template>
                                </div>
                                <template x-for="(d,i) in weekDays()" :key="'wc'+i">
                                    <div class="relative border-l border-gray-100 dark:border-gray-800" :style="`height:${24*hourPx}px`" @click="openNewAt(d,$event)">
            ...
            <div x-show="view==='day'" class="mt-4 overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
                ...
                <div class="max-h-[68vh] overflow-y-auto">
                    <div class="flex">
                        ...
                        <div class="relative flex-1 border-l border-gray-100 dark:border-gray-800" :style="`height:${24*hourPx}px`" @click="openNewAt(cursor,$event)">
            ...
            <div x-show="view==='agenda'" class="mt-4 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
                <ul class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <template x-if="agenda().length===0">
                        <li class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('calendar.ui.no_events') }}</li>
                    </template>
                </ul>
            </div>
        </div>
        ...
        <div x-show="editor" x-cloak class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4" role="dialog" @keydown.escape.window="editor=false">
            ...
            <div class="relative my-8 w-full max-w-lg rounded-lg bg-white dark:bg-gray-900 p-6 shadow-xl">
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100" x-text="form.read_only ? '{{ __('calendar.ui.view_event') }}' : (form.id ? '{{ __('calendar.ui.edit_event') }}' : '{{ __('calendar.ui.new_event') }}')"></h3>
                <p x-show="form.read_only" x-cloak class="mt-1 flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500"><x-icon name="lock-closed" class="h-3.5 w-3.5" /> {{ __('calendar.ui.read_only_note') }}</p>
                <div class="mt-4 space-y-3" :class="form.read_only && 'pointer-events-none opacity-80'">
                    ...
                    <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        <input type="checkbox" x-model="form.all_day" class="rounded border-gray-300 dark:border-gray-700"> {{ __('calendar.ui.all_day') }}
                    </label>
                    <div class="grid grid-cols-2 gap-2">
                        <label class="text-xs text-gray-500 dark:text-gray-400">{{ __('calendar.ui.start') }}
                            <input :type="form.all_day?'date':'datetime-local'" x-model="form.start" class="mt-0.5 w-full rounded-md border-gray-300 text-sm">
                        </label>
                        <label class="text-xs text-gray-500 dark:text-gray-400">{{ __('calendar.ui.end') }}
                    ...
                    <div x-show="!form.all_day">
                        <label class="text-xs text-gray-500 dark:text-gray-400">{{ __('calendar.ui.timezone') }}
                            <select x-model="form.timezone" class="mt-0.5 w-full rounded-md border-gray-300 dark:border-gray-700 text-sm">
                    ...
                    <template x-if="form.read_only">
                        <div class="text-sm text-gray-600 dark:text-gray-400" x-text="calName(form.calendar_id)"></div>
                    </template>
                </div>
                <div class="mt-5 flex items-center justify-between">
                    <button x-show="form.id && !form.read_only" @click="destroy()" class="text-sm text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">{{ __('calendar.ui.delete') }}</button>
                    <div class="ml-auto flex gap-2">
                        <button @click="editor=false" class="rounded-md border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800" x-text="form.read_only ? '{{ __('calendar.ui.close') }}' : '{{ __('calendar.ui.cancel') }}'"></button>
                        <button x-show="!form.read_only" @click="save()" class="rounded-md bg-gray-900 dark:bg-gray-100 dark:text-gray-900 px-3 py-2 text-sm font-semibold text-white hover:bg-gray-800 dark:hover:bg-white">{{ __('calendar.ui.save') }}</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="calModal.open" x-cloak class="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto p-4" role="dialog" @keydown.escape.window="calModal.open=false">
            ...
            <div class="relative my-16 w-full max-w-sm rounded-lg bg-white dark:bg-gray-900 p-5 shadow-xl">
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100" x-text="calModal.id ? '{{ __('calendar.ui.edit_calendar') }}' : '{{ __('calendar.ui.add_calendar') }}'"></h3>
                <form @submit.prevent="saveCalModal()" class="mt-3 space-y-3">
                    <template x-if="!calModal.readOnly">
                        <div>
                            <label class="text-xs text-gray-500 dark:text-gray-400">{{ __('calendar.ui.name') }}</label>
                            <input x-ref="calName" x-model="calModal.name" type="text" class="mt-0.5 w-full rounded-md border-gray-300 dark:border-gray-700 text-sm">
                        </div>
                    </template>
                    <p x-show="calModal.readOnly" x-cloak class="text-xs text-gray-500 dark:text-gray-400">{{ __('calendar.ui.color_only_note') }}</p>
                    <div>
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="calModal.open=false" class="rounded-md border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">{{ __('calendar.ui.cancel') }}</button>
                        <button type="submit" class="rounded-md bg-gray-900 dark:bg-gray-100 dark:text-gray-900 px-3 py-2 text-sm font-semibold text-white hover:bg-gray-800 dark:hover:bg-white">{{ __('calendar.ui.save') }}</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="linkModal.open" x-cloak class="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto p-4" role="dialog" @keydown.escape.window="linkModal.open=false">
            ...
            <div class="relative my-16 w-full max-w-sm rounded-lg bg-white dark:bg-gray-900 p-5 shadow-xl">
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100" x-text="linkModal.mode==='subscribe' ? '{{ __('calendar.ui.subscribe_title') }}' : '{{ __('calendar.ui.import_url_title') }}'"></h3>
                <form @submit.prevent="saveLinkModal()" class="mt-3 space-y-3">
                    <div>
                        <label class="text-xs text-gray-500 dark:text-gray-400">{{ __('calendar.ui.url') }}</label>
                        <input x-model="linkModal.url" type="url" placeholder="https://…" class="mt-0.5 w-full rounded-md border-gray-300 dark:border-gray-700 text-sm">
                    </div>
                    <div x-show="linkModal.mode==='subscribe'">
                        <label class="text-xs text-gray-500 dark:text-gray-400">{{ __('calendar.ui.name') }}</label>
                        <input x-model="linkModal.name" type="text" class="mt-0.5 w-full rounded-md border-gray-300 dark:border-gray-700 text-sm">
                    </div>
                    <p x-show="linkModal.error" x-cloak class="text-xs text-red-600 dark:text-red-400" x-text="linkModal.error"></p>
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="linkModal.open=false" class="rounded-md border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">{{ __('calendar.ui.cancel') }}</button>
                        <button type="submit" class="rounded-md bg-gray-900 dark:bg-gray-100 dark:text-gray-900 px-3 py-2 text-sm font-semibold text-white hover:bg-gray-800 dark:hover:bg-white">{{ __('calendar.ui.save') }}</button>
                    </div>
                </form>
            </div>
        ...
        <div x-show="confirmModal.open" x-cloak class="fixed inset-0 z-[70] flex items-start justify-center overflow-y-auto p-4" role="dialog" @keydown.escape.window="confirmModal.open=false">
            ...
            <div class="relative my-24 w-full max-w-sm rounded-lg bg-white dark:bg-gray-900 p-5 shadow-xl">
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('calendar.ui.confirm_delete_title') }}</h3>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400" x-text="confirmModal.message"></p>
                <div class="mt-4 flex justify-end gap-2">
                    <button type="button" @click="confirmModal.open=false" class="rounded-md border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">{{ __('calendar.ui.cancel') }}</button>
