                        ...
                        <div class="flex shrink-0 items-center gap-2" x-data="{ menu: false, sub: null }">
                            ...
                            <div class="relative">
                                ...
                                <div x-show="menu" x-cloak @click.outside="menu = false; sub = null" class="absolute right-0 z-20 mt-1 w-56 rounded-md border border-gray-200 bg-white py-1 text-left text-sm shadow-lg">
                                    <button type="button" @click="togglePin(current); menu = false" class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-gray-700 hover:bg-gray-50"><x-icon name="bookmark-solid" x-show="current.pinned" x-cloak /><x-icon name="bookmark" x-show="! current.pinned" /><span x-text="current.pinned ? @js(__('notes.unpin')) : @js(__('notes.pin'))"></span></button>
