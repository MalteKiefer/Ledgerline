  ...
  <div x-data="bookmarks({
    ...
    <template x-if="state === 'ready'">
      <div>
        <x-page-heading :title="__('bookmarks.title')">
            <x-slot:actions>
                <x-button variant="primary" icon="plus" @click="newBookmark()">{{ __('bookmarks.new_bookmark') }}</x-button>
            </x-slot:actions>
        </x-page-heading>
        ...
        <x-sheet side="left" store="sidebarOpen" :title="__('common.sections')">
            ...
            <section class="min-w-0 flex-1">
                <div class="flex items-center gap-2">
                    ...
                    <x-button variant="danger" class="shrink-0" x-show="view === 'trash' && trashCount" @click="emptyTrash()">{{ __('bookmarks.empty_trash') }}</x-button>
                </div>
        ...
        <div x-show="folderEditor.open" x-cloak class="fixed inset-0 z-[1050] flex items-center justify-center p-4" role="dialog" aria-modal="true" @keydown.escape.window="folderEditor.open = false">
            ...
            <div class="relative w-full max-w-sm rounded-2xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-6 shadow-xl">
                ...
                <div class="mt-5 flex justify-end gap-3">
                    <x-button variant="secondary" @click="folderEditor.open = false">{{ __('common.cancel') }}</x-button>
                    <x-button variant="primary" @click="saveFolder()">{{ __('common.save') }}</x-button>
                </div>
            </div>
        </div>
    </template>
        ...
        <div x-show="moreIconsOpen" x-cloak class="fixed inset-0 z-[1060] flex items-center justify-center p-4" role="dialog" aria-modal="true" @keydown.escape.window="moreIconsOpen = false">
            ...
            <div class="relative w-full w-[50vw] max-w-[50vw] rounded-2xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-6 shadow-xl">
                ...
                <div class="mt-5 flex justify-end">
                    <x-button variant="secondary" @click="moreIconsOpen = false">{{ __('common.close') }}</x-button>
                </div>
            </div>
        </div>
    </template>
    ...
    <template x-teleport="body">
        <div x-show="editorOpen" x-cloak class="fixed inset-0 z-[1050] flex items-center justify-center p-4" role="dialog" aria-modal="true" @keydown.escape.window="closeEditor()">
            ...
            <div class="relative flex max-h-[92vh] w-full w-[50vw] max-w-[50vw] flex-col rounded-2xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] shadow-xl" x-show="editing">
                ...
                <div class="min-h-0 flex-1 space-y-4 overflow-auto p-5" x-show="editing">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <x-tag-field :placeholder="__('bookmarks.tags_placeholder')" />
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-end gap-3 border-t border-gray-100 dark:border-gray-800 px-5 py-3">
                    <x-button variant="secondary" @click="closeEditor()">{{ __('bookmarks.cancel') }}</x-button>
                    <x-button variant="primary" @click="saveBookmark()">{{ __('bookmarks.save') }}</x-button>
                </div>
            </div>
        </div>
    </template>
