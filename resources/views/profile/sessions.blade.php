<x-layouts.app :title="__('account.nav_sessions')">
    <div class="mx-auto w-full max-w-3xl">
        ...
        <div class="mt-5 ll-card flex items-center gap-3.5">
            ...
            <span class="text-sm text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.last_login') }}</span>
            <span class="ml-auto text-sm text-md-on-surface dark:text-md-on-surface">{{ $user->last_login_at?->format('Y-m-d H:i') ?: __('account.never') }}</span>
        </div>

        <h2 class="mt-6 mb-2 px-1 text-xs font-semibold uppercase tracking-wide text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.sessions_heading') }}</h2>
            ...
            @forelse ($sessions as $s)
                <div class="flex items-center gap-3.5 px-4 py-3">
                    ...
                    <div class="min-w-0 flex-1">
                        <p class="truncate text-sm font-medium text-md-on-surface dark:text-md-on-surface">{{ $s['agent'] ?: __('account.sessions_unknown') }}</p>
                    ...
                    @unless ($s['current'])
                        <form method="POST" action="{{ route('account.sessions.revoke', $s['id']) }}" class="shrink-0">
                            @csrf @method('DELETE')
                            <x-icon-button name="x-mark" tone="red" type="submit" title="{{ __('account.sessions_revoke') }}" aria-label="{{ __('account.sessions_revoke') }}" />
                        </form>
                    @endunless
                </div>
            @empty
                <p class="px-4 py-3 text-sm text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.sessions_none') }}</p>
            @endforelse
        </div>
    </div>
</x-layouts.app>
