<x-layouts.app :title="__('account.nav_appearance')">
    <div class="mx-auto w-full max-w-3xl">
        ...
        {{-- Colour scheme --}}
        <h2 class="mt-5 mb-2 px-1 text-xs font-semibold uppercase tracking-wide text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.appearance_theme') }}</h2>
        <div class="ll-card">
                ...
                @foreach (['light' => 'sun', 'dark' => 'moon', 'system' => 'computer-desktop'] as $mode => $icon)
                    <form method="POST" action="{{ route('theme.update') }}" class="flex-1">
                        ...
                        <button type="submit" @class([
                            ...
                            'text-md-on-surface-var dark:text-md-on-surface-var hover:text-accent' => $theme !== $mode,
                        ])>
        ...
        {{-- Interface language --}}
        <h2 class="mt-6 mb-2 px-1 text-xs font-semibold uppercase tracking-wide text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.appearance_language') }}</h2>
            ...
            @foreach (config('locales.languages') as $code => $label)
                <form method="POST" action="{{ route('locale.update') }}">
                    ...
                    <button type="submit" class="group flex w-full items-center gap-3.5 px-4 py-3.5 text-left transition hover:bg-accent/5">
                        ...
                        <span class="flex-1 text-sm font-medium text-md-on-surface dark:text-md-on-surface">{{ $label }}</span>
        ...
        @endphp
        <h2 class="mt-6 mb-2 px-1 text-xs font-semibold uppercase tracking-wide text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.appearance_units') }}</h2>
        <div class="ll-card !p-0 overflow-hidden divide-y divide-black/[0.06] dark:divide-white/10">
            @foreach ($prefGroups as $key => $group)
                <div class="flex items-center justify-between gap-3 px-4 py-3">
                    <span class="text-sm font-medium text-md-on-surface dark:text-md-on-surface">{{ $group['label'] }}</span>
                    <div class="flex gap-1 rounded-xl bg-black/5 dark:bg-white/5 p-1">
                        @foreach ($group['options'] as $value => $optLabel)
                            <form method="POST" action="{{ route('preferences.update') }}">
                                ...
                                <button type="submit" @class([
                                    ...
                                    'text-md-on-surface-var dark:text-md-on-surface-var hover:text-accent' => ($prefs[$key] ?? '') !== $value,
                                ])>{{ $optLabel }}</button>
                            </form>
                        @endforeach
                    </div>
