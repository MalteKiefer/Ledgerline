<x-layouts.app :title="__('account.nav_devices')">
    <div class="mx-auto w-full max-w-3xl" x-data="devicePairing({ rateLimited: @js(__('account.pair_rate_limited')), startFailed: @js(__('account.pair_start_failed')) })">
        @include('profile._header', ['title' => __('account.nav_devices'), 'subtitle' => __('account.devices_hint').' '.__('account.devices_limit_note', ['max' => $deviceMax])])

        {{-- Connect: mobile app (QR) or command-line (code) --}}
        <div class="mt-5 ll-card">
            <template x-if="!active">
                <div class="flex flex-wrap gap-2">
                    <x-button variant="secondary" icon="qr-code" x-on:click="start('app')">{{ __('account.devices_connect') }}</x-button>
                    <x-button variant="secondary" icon="command-line" x-on:click="start('cli')">{{ __('account.cli_connect') }}</x-button>
                </div>
            </template>

            <template x-if="active">
                <div class="text-sm">
                    <div x-show="method==='app' && (status==='pending_scan' || status==='pending_approval')" class="flex flex-col items-start gap-4 sm:flex-row">
                        <img :src="qr" alt="" class="h-40 w-40 shrink-0 rounded-xl border border-black/[0.08] dark:border-white/10 bg-white p-1">
                        <p x-show="status==='pending_scan'" class="text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.devices_scan_hint') }}</p>
                    </div>
                    <div x-show="method==='cli' && (status==='pending_scan' || status==='pending_approval')">
                        <p class="text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.cli_paste_hint') }}</p>
                        <div class="mt-2 flex items-center gap-2">
                            <code x-text="code" class="min-w-0 flex-1 truncate rounded-xl border border-black/[0.08] dark:border-white/10 bg-md-surface-2 dark:bg-md-surface-2 px-3 py-2 font-mono text-sm text-md-on-surface dark:text-md-on-surface"></code>
                            <x-button variant="secondary" icon="clipboard" x-on:click="copyCode()" class="shrink-0">
                                <span x-text="copied ? @js(__('account.cli_copied')) : @js(__('account.cli_copy'))"></span>
                            </x-button>
                        </div>
                    </div>
                    <div x-show="status==='pending_scan' || status==='pending_approval'">
                        <p class="mt-2 text-xs text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.devices_expires_in') }} <span x-text="remainingText" class="font-mono font-medium text-md-on-surface-var dark:text-md-on-surface-var"></span></p>
                        <div x-show="status==='pending_approval'" class="mt-3">
                            <p class="text-md-on-surface dark:text-md-on-surface">{{ __('account.devices_approve_q') }} „<span x-text="deviceName" class="font-medium"></span>"?</p>
                            <div class="mt-2 flex gap-2">
                                <x-button variant="primary" x-on:click="approve()">{{ __('account.devices_allow') }}</x-button>
                                <x-button variant="secondary" x-on:click="reject()">{{ __('account.devices_deny') }}</x-button>
                            </div>
                        </div>
                        <button type="button" x-on:click="regenerate()" class="mt-3 inline-flex items-center gap-1.5 text-xs text-md-on-surface-var hover:text-md-on-surface-var dark:hover:text-md-on-surface-var"><x-icon name="arrow-path" class="h-3.5 w-3.5" />{{ __('account.devices_regenerate') }}</button>
                    </div>
                    <p x-show="status==='approved' || status==='consumed'" class="font-medium text-md-on-surface dark:text-md-on-surface">{{ __('account.devices_connected') }}</p>
                    <p x-show="status==='rejected'" class="text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.devices_rejected') }}</p>
                    <p x-show="status==='expired'" class="text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.devices_expired') }}</p>
                    <button type="button" x-on:click="reset()" x-show="['approved','consumed','rejected','expired'].includes(status)" class="mt-2 text-xs text-md-on-surface-var underline">{{ __('account.devices_again') }}</button>
                </div>
            </template>
        </div>

        {{-- Paired devices (app + CLI share one list) --}}
        <h2 class="mt-6 mb-2 px-1 text-xs font-semibold uppercase tracking-wide text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.devices_list_heading') }}</h2>
        <div class="ll-card !p-0 overflow-hidden divide-y divide-black/[0.06] dark:divide-white/10" x-show="devices.length">
            <template x-for="d in devices" :key="d.id">
                <div class="flex items-center gap-3.5 px-4 py-3">
                    <span class="ll-chip h-9 w-9 shrink-0" style="--chip: #3b9fd6"><x-icon name="device-phone-mobile" class="h-5 w-5" /></span>
                    <div class="min-w-0 flex-1">
                        <p class="flex items-center gap-1.5 truncate text-sm font-medium text-md-on-surface dark:text-md-on-surface">
                            <span class="truncate" x-text="d.name"></span>
                            <span x-show="d.syncing" class="inline-flex shrink-0 items-center gap-1 rounded-full bg-green-100 dark:bg-green-900/40 px-1.5 py-0.5 text-[11px] font-medium text-green-700 dark:text-green-300"><span class="h-1.5 w-1.5 rounded-full bg-green-500"></span>{{ __('account.devices_syncing') }}</span>
                            <span x-show="d.wipeRequested" class="shrink-0 rounded-full bg-red-100 dark:bg-red-900/40 px-1.5 py-0.5 text-[11px] font-medium text-red-700 dark:text-red-300">{{ __('account.devices_wipe_pending') }}</span>
                        </p>
                        <p class="truncate text-xs text-md-on-surface-var dark:text-md-on-surface-var" x-text="d.meta"></p>
                        <p x-show="d.version || d.installId" class="truncate text-xs text-md-on-surface-var dark:text-md-on-surface-var"
                           x-text="[d.version, d.installId ? '#' + d.installId : null].filter(Boolean).join(' · ')"></p>
                        <p x-show="d.syncing && d.syncDetail" class="truncate text-xs text-green-700 dark:text-green-400" x-text="d.syncDetail"></p>
                        <p x-show="!d.syncing && d.syncSeen" class="truncate text-xs text-md-on-surface-var" x-text="'{{ __('account.devices_last_sync') }} ' + d.syncSeen"></p>
                    </div>
                    <div class="flex shrink-0 items-center gap-1">
                        <x-icon-button name="exclamation-triangle" tone="red" x-on:click="wipeDevice(d.id)" x-show="!d.wipeRequested" title="{{ __('account.devices_wipe') }}" aria-label="{{ __('account.devices_wipe') }}" />
                        <x-icon-button name="trash" tone="red" x-on:click="revokeDevice(d.id)" title="{{ __('account.devices_revoke') }}" aria-label="{{ __('account.devices_revoke') }}" />
                    </div>
                </div>
            </template>
        </div>
        <p x-show="!devices.length" class="px-1 py-2 text-sm text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.devices_none') }}</p>
    </div>
</x-layouts.app>
