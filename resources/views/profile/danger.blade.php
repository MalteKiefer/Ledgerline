<x-layouts.app :title="__('account.delete_heading')">
    <div class="mx-auto w-full max-w-3xl" x-data="{ del: false }">
        ...
        <div class="mt-5 ll-card border-red-300 dark:border-red-800">
            <div class="flex items-start gap-3.5">
                ...
                <div class="min-w-0">
                    ...
                    <p class="mt-1 text-sm text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.delete_hint') }}</p>
                </div>
            </div>
        ...
        <div x-show="del" x-cloak class="fixed inset-0 z-[80] flex items-start justify-center overflow-y-auto p-4"
            ...
            <div class="relative my-16 w-full max-w-md rounded-xl bg-white dark:bg-md-surface-2 p-5 shadow-xl">
                <h3 class="text-base font-semibold text-md-on-surface dark:text-md-on-surface">{{ __('account.delete_modal_title') }}</h3>
                <p class="mt-2 text-sm text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.delete_modal_warning') }}</p>
                <form method="POST" action="{{ route('account.destroy') }}" class="mt-4 space-y-3">
                    @csrf @method('DELETE')
                    <label class="block text-sm text-md-on-surface-var dark:text-md-on-surface-var">{{ __('account.delete_confirm_label') }}
                        <input type="text" name="confirmation" required autocomplete="off"
                            class="mt-1 block w-full rounded-xl border-md-outline-variant dark:border-md-outline-variant text-sm shadow-sm focus:border-accent focus:ring-accent">
                    </label>
