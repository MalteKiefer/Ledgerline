...
<template x-teleport="body">
    <div x-show="$store.nav.{{ $store }}" x-cloak class="fixed inset-0 z-[70]" role="dialog" aria-modal="true"
        ...
        <div x-show="$store.nav.{{ $store }}"
            ...
            class="absolute {{ $panel }} overflow-y-auto bg-white p-4 shadow-xl dark:bg-md-surface-2">
            <div class="mb-3 flex items-center justify-between">
                <h2 class="text-sm font-semibold text-md-on-surface dark:text-md-on-surface">{{ $title ?? '' }}</h2>
                <button type="button" @click="$store.nav.{{ $store }} = false"
                    class="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md text-md-on-surface-var hover:bg-md-surface-2 dark:text-md-on-surface-var dark:hover:bg-gray-800"
