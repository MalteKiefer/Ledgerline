...
<div x-data="spotlight"
    ...
    <button type="button" @click="openPalette()" aria-label="{{ __('pages.spotlight.search_button_label') }}" title="{{ __('pages.spotlight.search_button_label') }}"
        class="inline-flex items-center justify-center rounded-md p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500">
    ...
    <template x-teleport="body">
        <div x-show="open" x-cloak class="fixed inset-0 z-50" role="dialog" aria-modal="true"
            ...
            <div class="relative mx-auto mt-[15vh] w-full max-w-xl px-4">
                <div class="overflow-hidden rounded-xl bg-white dark:bg-gray-900 shadow-2xl ring-1 ring-black/5">
                    <div class="flex items-center gap-2 border-b border-gray-100 dark:border-gray-800 px-4">
                        <svg class="h-5 w-5 shrink-0 text-gray-400 dark:text-gray-500" fill="none" viewBox="0 0 24 24"
                        ...
                        <span x-show="loading" x-cloak class="text-xs text-gray-400 dark:text-gray-500">…</span>
                    </div>

                    <div class="max-h-[70vh] overflow-y-auto">
                        <p x-show="query.trim() !== '' && flat.length === 0 && !loading" x-cloak
                            class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('pages.spotlight.no_results') }}</p>

                        <template x-for="group in groups" :key="group.group">
                            <div class="py-1">
                                <p class="px-4 pt-2 text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500"
                                    ...
                                    <template x-for="item in group.results" :key="item.url">
                                        <li>
                                            <a :href="item.url"
                                                ...
                                                :class="isActive(item) ? 'bg-gray-100 dark:bg-gray-800' : 'hover:bg-gray-50 dark:hover:bg-gray-800'">
                                                <span class="block truncate text-sm font-medium text-gray-900 dark:text-gray-100" x-text="item.title"></span>
                                                <span x-show="item.subtitle" class="block truncate text-xs text-gray-500 dark:text-gray-400"
                        ...
                        <a x-show="query.trim() !== '' && flat.length > 0" x-cloak href="#" @click.prevent="seeAll()"
                            class="block border-t border-gray-100 dark:border-gray-800 px-4 py-3 text-center text-xs text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800">
