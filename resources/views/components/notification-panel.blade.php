...
<div x-show="open" x-cloak
    class="absolute right-0 z-40 mt-2 w-80 max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-md border border-gray-200 bg-white shadow-lg">
    ...
    <div x-show="desktop !== 'granted' && desktop !== 'unsupported'" x-cloak class="border-b border-gray-100 px-3 py-2">
        <button type="button" @click="enableDesktop()" class="text-xs font-medium text-gray-700 hover:text-gray-900">{{ __('notifications.enable_desktop') }}</button>
    </div>
    <div class="max-h-96 overflow-y-auto">
        ...
        <template x-for="n in items" :key="n.id">
            <button type="button" @click="activate(n)" class="flex w-full items-start gap-2 border-b border-gray-50 px-3 py-2 text-left hover:bg-gray-50" :class="[! n.read ? 'bg-gray-50' : '', hrefFor(n) ? 'cursor-pointer' : '']">
