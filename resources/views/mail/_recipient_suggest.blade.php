...
<div x-show="recipientBox.field === '{{ $field }}' && recipientBox.items.length" x-cloak
    class="absolute left-0 right-0 top-full z-10 mt-1 max-h-60 overflow-auto rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 py-1 text-sm shadow-lg"
    ...
    <template x-for="(r, i) in recipientBox.items" :key="r.email + i">
        <button type="button"
            ...
            class="flex w-full flex-col items-start gap-0.5 px-3 py-1.5 text-left hover:bg-gray-50 dark:hover:bg-gray-800"
            :class="recipientBox.active === i ? 'bg-gray-50 dark:bg-gray-800' : ''"
            role="option" :aria-selected="recipientBox.active === i">
            <span class="truncate font-medium text-gray-900 dark:text-gray-100" x-text="r.name || r.email"></span>
            <span class="truncate text-xs text-gray-500 dark:text-gray-400" x-show="r.name" x-text="r.email"></span>
        </button>
    </template>
</div>
