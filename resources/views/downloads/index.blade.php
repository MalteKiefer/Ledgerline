<x-layouts.app :title="__('downloads.heading')">
    ...
    <div x-data="downloadsPage(@js($dlLabels))" x-init="init()">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">{{ __('downloads.heading') }}</h1>
                <p class="mt-1 text-sm text-gray-500">{{ __('downloads.subheading') }}</p>
            </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="font-medium text-gray-900" x-text="e.title"></span>
                                <span class="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-600" x-text="sourceLabel(e)"></span>
                                <span class="rounded px-2 py-0.5 text-xs font-medium"
                                    :class="{
                                        'bg-gray-100 text-gray-600': e.status==='queued',
                                        'bg-blue-100 text-blue-700': e.status==='processing',
                                        'bg-green-100 text-green-700': e.status==='ready',
                            ...
                            <template x-if="e.status==='processing' || e.status==='queued'">
                                <p class="mt-2 text-xs text-gray-400">{{ __('downloads.building_hint') }}</p>
                            </template>
                            <template x-if="e.status==='failed'">
                                <p class="mt-2 text-xs text-red-600" x-text="e.error"></p>
                            </template>
                        </div>
                        <button type="button" @click="destroy(e.id)" :title="'{{ __('downloads.delete') }}'"
                            class="shrink-0 rounded p-1.5 text-gray-400 hover:bg-gray-50 hover:text-red-600">
