...
<div>
    <label class="block text-xs font-medium text-md-on-surface-var dark:text-md-on-surface-var mb-1">{{ __('settings.users_name') }}</label>
...
<div>
    <label class="block text-xs font-medium text-md-on-surface-var dark:text-md-on-surface-var mb-1">{{ __('settings.users_email') }}</label>
    <input type="email" name="email" required value="{{ old('email', $user?->email) }}"
        class="w-full rounded-lg border border-md-outline-variant dark:border-md-outline-variant bg-white dark:bg-md-surface px-3 py-2 text-sm text-md-on-surface dark:text-md-on-surface focus:border-accent focus:ring-accent">
</div>
<div>
    <label class="block text-xs font-medium text-md-on-surface-var dark:text-md-on-surface-var mb-1">{{ __('settings.users_role') }}</label>
    <select name="role" class="w-full rounded-lg border border-md-outline-variant dark:border-md-outline-variant bg-white dark:bg-md-surface px-3 py-2 text-sm text-md-on-surface dark:text-md-on-surface focus:border-accent focus:ring-accent">
...
@if ($isNew)
    <div x-data="pwStrength(@js([__('settings.users_pw_s0'), __('settings.users_pw_s1'), __('settings.users_pw_s2'), __('settings.users_pw_s3'), __('settings.users_pw_s4')]))">
        <label class="block text-xs font-medium text-md-on-surface-var dark:text-md-on-surface-var mb-1">{{ __('settings.users_password') }}</label>
        ...
        <p class="mt-1 text-[11px] text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.users_password_hint') }}</p>
    </div>
@endif
...
@if (! ($groups ?? collect())->isEmpty())
    ...
    @endphp
    <div class="sm:col-span-2">
        <label class="block text-xs font-medium text-md-on-surface-var dark:text-md-on-surface-var mb-1">{{ __('settings.users_groups') }}</label>
