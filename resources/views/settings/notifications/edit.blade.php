<x-layouts.app :title="__('settings.notifications_heading')">
    ...
    <h1 class="mt-1 text-2xl font-semibold text-gray-900">{{ __('settings.notifications_heading') }}</h1>
    ...
    <form method="POST" action="{{ route('settings.notifications.update') }}" class="mt-6 space-y-4">
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <p class="mt-1 text-xs text-gray-500">{{ __('settings.notify_mail_hint') }}</p>
            <div class="mt-3 grid gap-3 sm:grid-cols-2">
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.smtp_host') }}</label>
                    <input type="text" name="smtp_host" value="{{ old('smtp_host', $settings->smtp_host) }}" class="{{ $input }}">
                    @error('smtp_host')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.smtp_port') }}</label>
                    <input type="number" name="smtp_port" value="{{ old('smtp_port', $settings->smtp_port) }}" class="{{ $input }}"></div>
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.smtp_encryption') }}</label>
                ...
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.smtp_username') }}</label>
                    <input type="text" name="smtp_username" value="{{ old('smtp_username', $settings->smtp_username) }}" class="{{ $input }}" autocomplete="off"></div>
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.smtp_password') }}</label>
                    <input type="password" name="smtp_password" value="" class="{{ $input }}" autocomplete="new-password" placeholder="••••••••">
                    <p class="mt-1 text-xs text-gray-500">{{ __('settings.notify_secret_keep_hint') }}</p></div>
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.smtp_from_address') }}</label>
                    <input type="email" name="smtp_from_address" value="{{ old('smtp_from_address', $settings->smtp_from_address) }}" class="{{ $input }}">
                    @error('smtp_from_address')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.smtp_from_name') }}</label>
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-3 grid gap-3 sm:grid-cols-2">
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.ntfy_url') }}</label>
                    <input type="url" name="ntfy_url" value="{{ old('ntfy_url', $settings->ntfy_url) }}" placeholder="https://ntfy.sh" class="{{ $input }}">
                    @error('ntfy_url')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.ntfy_topic') }}</label>
                    <input type="text" name="ntfy_topic" value="{{ old('ntfy_topic', $settings->ntfy_topic) }}" class="{{ $input }}"></div>
                <div class="sm:col-span-2"><label class="block text-sm font-medium text-gray-700">{{ __('settings.ntfy_token') }}</label>
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-3 grid gap-3 sm:grid-cols-2">
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.webhook_url') }}</label>
                    <input type="url" name="webhook_url" value="{{ old('webhook_url', $settings->webhook_url) }}" class="{{ $input }}">
                    @error('webhook_url')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.webhook_secret') }}</label>
