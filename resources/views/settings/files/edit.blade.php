<x-layouts.app :title="__('settings.files_section')">
    ...
    <form method="POST" action="{{ route('settings.files.update') }}" class="mt-6 ll-card">
        @csrf
        @method('PUT')
        <label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var" for="file_max_versions">{{ __('settings.files_max_versions') }}</label>
        <p class="mt-1 text-xs text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.files_max_versions_hint') }}</p>
        <select id="file_max_versions" name="file_max_versions" class="mt-2 block w-full rounded-md border-md-outline-variant dark:border-md-outline-variant text-sm shadow-sm focus:border-accent focus:ring-accent sm:w-32">
