...
<form method="POST" action="{{ $action }}"
    x-data="{
        ...
        get isMirror() { return this.source === 'files' || this.source === 'gallery'; },
    }" class="space-y-3">
    @csrf
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        ...
        <div><label class="block text-sm font-medium text-gray-700">{{ __('settings.backup_source') }}</label>
            ...
            <p x-show="isMirror" x-cloak class="mt-1 text-xs text-gray-500">{{ __('settings.backup_mirror_hint') }}</p></div>
        ...
        <div x-show="! isMirror"><label class="block text-sm font-medium text-gray-700">{{ __('settings.backup_retention') }}</label>
    ...
    <div class="flex flex-wrap items-start gap-6">
        <label x-show="! isMirror" class="flex items-center gap-2 text-sm text-gray-700">
        ...
        <div x-show="encrypt && ! isMirror" class="min-w-[16rem]">
