...
<form method="POST" action="{{ $action }}"
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        ...
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_driver') }}</label>
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_bucket') }}</label>
            <input type="text" name="bucket" value="{{ old('bucket', $cfg['bucket'] ?? '') }}" class="{{ $input }}"></div>
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_region') }}</label>
            <input type="text" name="region" value="{{ old('region', $cfg['region'] ?? '') }}" class="{{ $input }}"></div>
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_key') }}</label>
            <input type="text" name="key" value="{{ old('key', $cfg['key'] ?? '') }}" class="{{ $input }}" autocomplete="off"></div>
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_secret') }}</label>
            <input type="password" name="secret" value="" class="{{ $input }}" autocomplete="off" placeholder="••••••••">
            @if ($d)<p class="mt-1 text-xs text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.notify_secret_keep_hint') }}</p>@endif</div>
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_endpoint') }}</label>
            <input type="text" name="endpoint" value="{{ old('endpoint', $cfg['endpoint'] ?? '') }}" class="{{ $input }}"></div>
        <label class="flex items-center gap-2 pt-6 text-sm text-md-on-surface-var dark:text-md-on-surface-var">
            <input type="checkbox" name="use_path_style" value="1" @checked(old('use_path_style', $cfg['use_path_style'] ?? false)) class="rounded border-md-outline-variant dark:border-md-outline-variant focus:ring-accent">
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_host') }}</label>
            <input type="text" name="host" value="{{ old('host', $cfg['host'] ?? '') }}" class="{{ $input }}"></div>
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_port') }}</label>
            <input type="number" name="port" value="{{ old('port', $cfg['port'] ?? 22) }}" class="{{ $input }}"></div>
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_username') }}</label>
            <input type="text" name="username" value="{{ old('username', $cfg['username'] ?? '') }}" class="{{ $input }}" autocomplete="off"></div>
        <div><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_password') }}</label>
            <input type="password" name="password" value="" class="{{ $input }}" autocomplete="off" placeholder="••••••••">
            @if ($d)<p class="mt-1 text-xs text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.notify_secret_keep_hint') }}</p>@endif</div>
        <div class="sm:col-span-2"><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_path') }}</label>
            <input type="text" name="path" value="{{ old('path', $cfg['path'] ?? '') }}" class="{{ $input }}">
            <p class="mt-1 text-xs text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_path_hint') }}</p></div>
    </div>
    </template>
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        <div class="sm:col-span-2"><label class="block text-sm font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.backup_base_uri') }}</label>
    ...
    <div class="flex flex-wrap gap-2">
        <x-button variant="secondary" @click="testConn($event)" ::disabled="testing">
