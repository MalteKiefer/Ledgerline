<x-layouts.app :title="__('settings.paperless_heading')">
    ...
    <h1 class="mt-1 text-2xl font-semibold text-gray-900">{{ __('settings.paperless_heading') }}</h1>
    <p class="mt-1 text-sm text-gray-600">{{ __('settings.paperless_desc') }}</p>
    ...
    <div x-data="paperlessSettings({
        ...
        <form method="POST" action="{{ route('settings.paperless.update') }}" class="mt-6 rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
            ...
            <label class="flex items-center gap-2">
                ...
                <span class="text-sm font-medium text-gray-700">{{ __('settings.paperless_enabled') }}</span>
            </label>

            <div class="mt-4">
                <label for="paperless_url" class="block text-sm font-medium text-gray-700">{{ __('settings.paperless_url') }}</label>
                ...
                @error('paperless_url')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div class="mt-4">
                <label for="paperless_token" class="block text-sm font-medium text-gray-700">{{ __('settings.paperless_token') }}</label>
                ...
                <p class="mt-1 text-xs text-gray-500">{{ __('settings.paperless_token_hint') }}</p>
        ...
        <div class="mt-6 rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
                ...
                <div class="rounded-md bg-gray-50 p-3">
                    <div class="text-xs text-gray-500">{{ __('settings.paperless_document_types') }}</div>
                    <div class="mt-1 text-xl font-semibold text-gray-900" x-text="counts.document_type"></div>
                </div>
                <div class="rounded-md bg-gray-50 p-3">
                    <div class="text-xs text-gray-500">{{ __('settings.paperless_correspondents') }}</div>
                    <div class="mt-1 text-xl font-semibold text-gray-900" x-text="counts.correspondent"></div>
                </div>
            </div>
            @if ($settings->paperless_synced_at)
                <p class="mt-3 text-xs text-gray-400">{{ __('settings.paperless_synced_at', ['time' => $settings->paperless_synced_at->timezone(config('app.timezone'))->format('Y-m-d H:i')]) }}</p>
            @else
                <p class="mt-3 text-xs text-gray-400">{{ __('settings.paperless_never_synced') }}</p>
            @endif
            <p x-show="syncError" x-cloak class="mt-2 text-sm text-red-600" x-text="syncError"></p>
        </div>
    </div>
</x-layouts.app>
