    ...
    <form method="GET" action="{{ route('settings.security-log') }}" class="mt-6 ll-card">
        <div class="grid gap-3 sm:grid-cols-4">
            <label class="block text-sm">
                <span class="mb-1 block text-xs font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.seclog_filter_action') }}</span>
                <select name="action" class="w-full rounded-xl border-md-outline-variant dark:border-md-outline-variant text-sm focus:border-accent focus:ring-accent">
            ...
            <label class="block text-sm">
                <span class="mb-1 block text-xs font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.seclog_filter_user') }}</span>
                <input type="number" name="user" value="{{ $filters['user'] }}" min="1"
                    class="w-full rounded-xl border-md-outline-variant dark:border-md-outline-variant text-sm focus:border-accent focus:ring-accent">
            </label>
            <label class="block text-sm">
                <span class="mb-1 block text-xs font-medium text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.seclog_filter_since') }}</span>
    ...
    <div class="mt-6 ll-card !p-0 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-black/[0.06] dark:border-white/10 text-xs text-md-on-surface-var dark:text-md-on-surface-var">
                ...
                <tbody>
                    @forelse ($logs as $log)
                        <tr class="border-b border-black/[0.04] dark:border-white/[0.06]">
                            <td class="whitespace-nowrap px-4 py-2.5 text-md-on-surface-var dark:text-md-on-surface-var">{{ $log->created_at?->format('Y-m-d H:i:s') }}</td>
                            <td class="whitespace-nowrap px-4 py-2.5 text-md-on-surface-var dark:text-md-on-surface-var">{{ $log->actor?->name ?? ($log->user_id ?? '—') }}</td>
                            ...
                            <td class="whitespace-nowrap px-4 py-2.5 font-mono text-xs text-md-on-surface-var dark:text-md-on-surface-var">{{ $log->ip ?? '' }}</td>
                            <td class="max-w-md truncate px-4 py-2.5 font-mono text-xs text-md-on-surface-var dark:text-md-on-surface-var" title="{{ json_encode($log->meta) }}">{{ $log->meta ? json_encode($log->meta, JSON_UNESCAPED_SLASHES) : '' }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="px-4 py-8 text-center text-sm text-md-on-surface-var dark:text-md-on-surface-var">{{ __('settings.seclog_empty') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
