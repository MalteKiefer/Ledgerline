...
<div x-data="tagSelect(@js($ts))" @click.outside="open = false" class="relative">
    <div class="flex flex-wrap items-center gap-1.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#1c1c1e] px-2 py-1.5 focus-within:border-accent focus-within:ring-1 focus-within:ring-accent">
        ...
        <input type="text" x-model="q" @focus="open = true" @input="open = true"
            ...
            class="min-w-[8rem] flex-1 border-0 bg-transparent p-0.5 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-0">
    </div>
    <template x-for="id in selected" :key="'h'+id"><input type="hidden" :name="name" :value="id"></template>
    <div x-show="open && matches.length" x-cloak class="absolute z-20 mt-1 max-h-52 w-full overflow-auto rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1c1c1e] py-1 shadow-xl">
        <template x-for="o in matches" :key="o.id">
            <button type="button" @click="add(o.id)" class="flex w-full items-center justify-between gap-2 px-3 py-1.5 text-left text-sm hover:bg-accent/5">
                <span class="truncate text-gray-900 dark:text-gray-100" x-text="o.label"></span>
                <span class="truncate text-xs text-gray-400 dark:text-gray-500" x-text="o.sub"></span>
            </button>
        </template>
    </div>
</div>
