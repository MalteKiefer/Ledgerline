<x-layouts.app title="Time">
    <x-finance-nav />

    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Time</h1>
            <p class="mt-1 text-sm text-gray-600">Billable hours for customers and projects.</p>
        </div>
        <a href="{{ route('finance.time-entries.create') }}"
            class="rounded-md bg-gray-800 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700">New time entry</a>
    </div>

    <div class="mt-4 flex flex-wrap gap-3">
        @forelse ($totals as $currency => $cents)
            <div class="rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-sm">
                <span class="text-xs uppercase tracking-wide text-gray-400">Billable ({{ $currency }})</span>
                <div class="text-lg font-semibold text-gray-900">{{ number_format($cents / 100, 2) }} {{ $currency }}</div>
            </div>
        @empty
            <div class="rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm text-gray-500 shadow-sm">No time yet.</div>
        @endforelse
    </div>
    ...
    <form method="GET" class="mt-4 flex flex-wrap items-end gap-3">
        ...
        @if (request('q'))<a href="{{ route('finance.time-entries.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>@endif
    </form>

    <div class="mt-4 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        @if ($entries->isEmpty())
            <p class="px-4 py-8 text-center text-sm text-gray-500">No time entries.</p>
        @else
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right"><x-sortable-header column="minutes" label="Hours" :sort="$sort" :dir="$dir" /></th>
                        <th class="px-4 py-3 text-right"><x-sortable-header column="rate_cents" label="Rate" :sort="$sort" :dir="$dir" /></th>
                    ...
                    @foreach ($entries as $entry)
                        <tr>
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900">
                                ...
                                @unless ($entry->billable)<span class="ml-1 rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-600">Non-billable</span>@endunless
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('finance.time-entries.edit', $entry) }}" class="text-sm text-gray-500 hover:text-gray-900">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
