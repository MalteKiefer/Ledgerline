        ...
        <div class="mt-6 grid grid-cols-3 gap-4 sm:grid-cols-4 md:grid-cols-6">
            <template x-for="p in people" :key="p.id">
                <a :href="cfg.showBase + '/' + p.id" class="group block text-center">
                    <div class="aspect-square overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700">
                        <template x-if="p.cover">
                            <img :src="p.cover" alt="" class="h-full w-full object-cover" loading="lazy">
                        </template>
                    </div>
                    <p class="mt-2 truncate text-sm font-medium text-gray-800 dark:text-gray-200" x-text="p.name || '{{ __('gallery.person_unnamed') }}'"></p>
                    <p class="text-xs text-gray-400 dark:text-gray-500" x-text="p.count"></p>
                </a>
            </template>
        </div>
    </div>
