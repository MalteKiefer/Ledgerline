    ...
    <div class="flex flex-col gap-4 md:flex-row" x-data="albumsPage(@js($cfg))" x-init="init()">
        ...
        <div class="min-w-0 flex-1">
            ...
            <template x-if="!loading && !albums.length">
                <div class="mt-8 rounded-lg border border-dashed border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 p-10 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('gallery.no_albums') }}</div>
            </template>
                ...
                <template x-for="a in albums" :key="a.id">
                    <div class="group relative">
                        <a :href="cfg.showBase + '/' + a.id" class="block">
                            <div class="aspect-square overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700">
                                <template x-if="a.cover"><img :src="a.cover" alt="" class="h-full w-full object-cover" loading="lazy"></template>
                            </div>
                            <p class="mt-2 truncate text-sm font-medium text-gray-900 dark:text-gray-100" x-text="a.name"></p>
                            <p class="text-xs text-gray-400 dark:text-gray-500" x-text="'{{ __('gallery.album_photos_count', ['count' => '__C__']) }}'.replace('__C__', a.count)"></p>
                        </a>
                        <button type="button" x-show="a.owned" @click="openShare('albums', a.id, a.name)"
                            class="absolute right-1 top-1 hidden rounded-md bg-white/90 p-1.5 text-gray-600 dark:text-gray-400 shadow ring-1 ring-gray-200 dark:ring-gray-700 hover:text-gray-900 dark:hover:text-white group-hover:block"
        ...
        <div x-show="nameModal.open" x-cloak class="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto p-4" @keydown.escape.window="nameModal.open=false">
            ...
            <div class="relative my-16 w-full max-w-sm rounded-lg bg-white dark:bg-gray-900 p-5 shadow-xl">
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('gallery.new_album') }}</h3>
