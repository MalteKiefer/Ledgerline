...
<div class="space-y-2 border-t border-gray-100 pt-3">
    <label class="block cursor-pointer text-sm text-gray-600 hover:text-gray-900" :class="importing && 'pointer-events-none opacity-60'">
    ...
    <a href="{{ route('contacts.duplicates') }}" class="block text-sm text-gray-600 hover:text-gray-900">{{ __('contacts.ui.duplicates') }}</a>
    <div x-show="importing" x-cloak class="flex items-center gap-2 text-sm text-gray-500">
        <x-icon name="arrow-path" class="h-4 w-4 animate-spin" />
        <span>{{ __('contacts.ui.importing') }}</span>
    </div>
    <div x-show="importResult" x-cloak x-transition class="text-xs text-gray-600" x-text="importResult"></div>
</div>
