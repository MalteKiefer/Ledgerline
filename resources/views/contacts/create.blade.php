<x-layouts.app :title="__('contacts.create.title')">
    <p class="text-sm text-gray-500">
        <a href="{{ route('customers.show', $customer) }}" class="hover:underline">{{ $customer->name }}</a>
        <span aria-hidden="true">/</span> {{ __('contacts.create.breadcrumb') }}
    </p>
    <h1 class="mt-1 text-2xl font-semibold text-gray-900">{{ __('contacts.create.heading') }}</h1>
    ...
    <form method="POST" action="{{ route('customers.contacts.store', $customer) }}"
        ...
        <div class="mt-6 flex items-center gap-3">
            ...
            <a href="{{ route('customers.show', $customer) }}" class="text-sm text-gray-600 hover:text-gray-900">{{ __('contacts.create.cancel') }}</a>
        </div>
    </form>
</x-layouts.app>
