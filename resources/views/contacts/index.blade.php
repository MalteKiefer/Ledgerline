    ...
    <div x-data="contactsPage(@js($cfg))" x-init="init()" class="flex flex-col gap-4 md:flex-row">
        ...
        <div class="min-w-0 flex-1">
            ...
            <template x-if="!loading && contacts.length===0">
                <p class="mt-8 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('contacts.ui.empty') }}</p>
            </template>

            {{-- Bulk selection bar --}}
            <div x-show="selected.length" x-cloak class="mt-3 flex flex-wrap items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-4 py-2 text-sm shadow-sm">
                <span class="font-medium text-gray-900 dark:text-gray-100" x-text="@js(__('contacts.ui.selected_count')).replace(':count', selected.length)"></span>
                <button type="button" @click="toggleAll()" class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">{{ __('contacts.ui.select_all') }}</button>
                <button type="button" @click="selected = []" class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">{{ __('contacts.ui.clear_selection') }}</button>
            ...
            <ul class="mt-4 divide-y divide-gray-100 dark:divide-gray-800 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
                <template x-for="c in contacts" :key="c.id">
                    <li class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer" @click="openEditor(c.id)">
                        ...
                        <div class="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800 text-xs font-medium text-gray-500 dark:text-gray-400 ring-1 ring-gray-200 dark:ring-gray-700">
                            ...
                            <template x-if="! c.avatar && ! initials(c)"><x-icon name="user" class="h-4 w-4 text-gray-400 dark:text-gray-500" /></template>
                        </div>
                        <div class="min-w-0 flex-1">
                            <p class="truncate text-sm font-medium text-gray-900 dark:text-gray-100" x-text="displayName(c)"></p>
                            <p class="truncate text-xs text-gray-500 dark:text-gray-400" x-text="(c.org || '') + (c.emails[0] ? ' · '+c.emails[0] : '')"></p>
                        </div>
                        <button type="button" @click.stop="toggleFavorite(c)"
                            class="inline-flex min-h-9 min-w-9 shrink-0 items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
        ...
        <div x-show="nameModal.open" x-cloak class="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto p-4" role="dialog" @keydown.escape.window="nameModal.open=false">
            ...
            <div class="relative my-16 w-full max-w-sm rounded-lg bg-white dark:bg-gray-900 p-5 shadow-xl">
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100" x-text="nameModal.title"></h3>
                <form @submit.prevent="submitNameModal()">
                    ...
                    <div class="mt-4 flex justify-end gap-2">
                        <button type="button" @click="nameModal.open=false" class="rounded-md border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">{{ __('contacts.ui.cancel') }}</button>
                        <button type="submit" class="rounded-md bg-gray-900 dark:bg-gray-100 px-3 py-2 text-sm font-semibold text-white dark:text-gray-900 hover:bg-gray-800 dark:hover:bg-white">{{ __('contacts.ui.save') }}</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="confirmModal.open" x-cloak class="fixed inset-0 z-[70] flex items-start justify-center overflow-y-auto p-4" role="dialog" @keydown.escape.window="confirmModal.open=false">
            ...
            <div class="relative my-24 w-full max-w-sm rounded-lg bg-white dark:bg-gray-900 p-5 shadow-xl">
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('contacts.ui.confirm_title') }}</h3>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400" x-text="confirmModal.message"></p>
                <div class="mt-4 flex justify-end gap-2">
                    <button type="button" @click="confirmModal.open=false" class="rounded-md border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">{{ __('contacts.ui.cancel') }}</button>
