<x-layouts.app title="Contacts">
    ...
    <div class="mt-4">
        <x-table-search placeholder="Search contacts…" />
    </div>

    <div class="mt-4 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        @if ($contacts->isEmpty())
            <p class="px-4 py-8 text-center text-sm text-gray-500">No contacts found.</p>
        @else
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                    <tr>
                        <th scope="col" class="px-4 py-3"><x-sortable-header column="name" label="Name" :sort="$sort" :dir="$dir" /></th>
                        <th scope="col" class="px-4 py-3"><x-sortable-header column="function" label="Function" :sort="$sort" :dir="$dir" /></th>
