                <div class="rounded-xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-2 text-sm shadow-sm">
                    <button type="button" @click="view = 'all'; activeTag = ''" class="block w-full rounded px-3 py-1.5 text-left" :class="view === 'all' ? 'bg-accent/10 font-medium text-accent' : 'text-gray-700 dark:text-gray-300 hover:bg-accent/5'">{{ __('todos.all') }}</button>
                    <button type="button" @click="view = 'marked'; activeTag = ''" class="flex w-full items-center gap-2 rounded px-3 py-1.5 text-left" :class="view === 'marked' ? 'bg-accent/10 font-medium text-accent' : 'text-gray-700 dark:text-gray-300 hover:bg-accent/5'"><x-icon name="heart" class="h-4 w-4" />{{ __('todos.marked') }}</button>
                ...
                <div class="rounded-xl border border-black/[0.06] dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-2 text-sm shadow-sm">
                    ...
                    <template x-for="l in lists" :key="l.id">
                        <div class="group flex items-center justify-between rounded px-3 py-1.5" :class="view === l.id ? 'bg-accent/10' : 'hover:bg-accent/5'">
                            ...
                            <span class="flex shrink-0 items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100">
                                <x-icon-button name="pencil" tone="gray" size="sm" @click="renameList(l)" title="{{ __('todos.rename_list') }}" />
                                <x-icon-button name="trash" tone="gray" size="sm" @click="deleteList(l)" title="{{ __('todos.delete_list') }}" class="hover:!text-red-600" />
                            </span>
                        </div>
                    </template>
                    <form class="mt-1 flex items-center gap-1 px-1" @submit.prevent="addList()">
                        <input type="text" x-model="newListName" placeholder="{{ __('todos.new_list_placeholder') }}" class="w-full rounded-md border-gray-300 text-xs shadow-sm focus:border-accent focus:ring-accent">
                        <x-icon-button type="submit" name="plus" tone="gray" size="sm" title="{{ __('todos.add_list') }}" class="shrink-0" />
                    </form>
                </div>
