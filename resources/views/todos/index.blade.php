        ...
        <div class="mt-6 flex flex-col gap-4 md:flex-row" style="min-height: calc(100vh - 18rem);">
            ...
            <aside class="w-full shrink-0 space-y-4 md:w-64">
                <div class="rounded-lg border border-gray-200 bg-white p-2 text-sm shadow-sm">
                    <button type="button" @click="view = 'all'; activeTag = ''" class="block w-full rounded px-3 py-1.5 text-left" :class="view === 'all' ? 'bg-gray-100 font-medium text-gray-900' : 'text-gray-700 hover:bg-gray-50'">{{ __('todos.all') }}</button>
            ...
            <section class="min-w-0 flex-1">
                ...
                <p x-show="! filteredTasks.length" class="mt-10 text-center text-sm text-gray-500">{{ __('todos.empty') }}</p>
            </section>
        </div>
      </div>
    </template>
