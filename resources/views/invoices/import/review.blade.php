<x-layouts.app title="Review imported invoice">
    @php
    ...
    <x-finance-nav />
    <p class="text-sm text-gray-500"><a href="{{ route('finance.invoices.import.create') }}" class="hover:underline">Import</a></p>
    <h1 class="mt-1 text-2xl font-semibold text-gray-900">Review imported invoice</h1>
    <p class="mt-1 text-sm text-gray-600">
        Read from <strong>{{ $file->name }}</strong>.
        <a href="{{ route('files.download', $file) }}" target="_blank" rel="noopener" class="text-gray-900 underline">Open PDF ↗</a>.
        Parsed totals: net {{ number_format($parsed['net'] ?? 0, 2) }} · VAT {{ number_format($parsed['tax'] ?? 0, 2) }} · gross {{ number_format($parsed['gross'] ?? 0, 2) }}.
    </p>
    ...
    <form method="POST" action="{{ route('finance.invoices.import.store') }}" class="mt-6 space-y-6">
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label for="number" class="block text-sm font-medium text-gray-700">Invoice number<span class="text-red-600"> *</span></label>
                    ...
                    <p class="mt-1 text-xs text-gray-400">The internal counter continues from this number.</p>
                </div>
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm"
            ...
            <div class="mt-3 flex gap-4 text-sm">
                <label class="flex items-center gap-2"><input type="radio" name="customer_mode" value="existing" x-model="mode"> Existing</label>
                <label class="flex items-center gap-2"><input type="radio" name="customer_mode" value="new" x-model="mode"> Create new</label>
            </div>
            ...
            <div x-show="mode === 'new'" x-cloak class="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label for="new_customer_name" class="block text-sm font-medium text-gray-700">Name<span class="text-red-600"> *</span></label>
                ...
                <div>
                    <label for="new_customer_vat_id" class="block text-sm font-medium text-gray-700">VAT ID</label>
                    <input type="text" id="new_customer_vat_id" name="new_customer_vat_id" value="{{ old('new_customer_vat_id', $c['vat_id']) }}" class="{{ $input }}">
                </div>
                <div>
                    <label for="new_customer_street" class="block text-sm font-medium text-gray-700">Street</label>
                    <input type="text" id="new_customer_street" name="new_customer_street" value="{{ old('new_customer_street', $c['street']) }}" class="{{ $input }}">
                </div>
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label for="new_customer_postal_code" class="block text-sm font-medium text-gray-700">Postal code</label>
                    ...
                    <div>
                        <label for="new_customer_city" class="block text-sm font-medium text-gray-700">City</label>
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="rounded-md bg-gray-800 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700">Import invoice</button>
            <a href="{{ route('finance.invoices.import.create') }}" class="text-sm text-gray-600 hover:text-gray-900">Cancel</a>
        </div>
    </form>
</x-layouts.app>
