...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
    <div>
        <label for="date" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.date') }}<span class="text-red-600"> *</span></label>
        <input type="date" id="date" name="date" value="{{ old('date', $expense->date?->format('Y-m-d')) }}" required class="{{ $input }}">
        @error('date')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
    </div>
    <div>
        <label for="vendor" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.vendor') }}</label>
        <input type="text" id="vendor" name="vendor" value="{{ old('vendor', $expense->vendor) }}" class="{{ $input }}">
    </div>

    <div class="sm:col-span-2">
        <label for="description" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.description') }}<span class="text-red-600"> *</span></label>
    ...
    <div>
        <label for="category" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.category') }}<span class="text-red-600"> *</span></label>
        <select id="category" name="category" required class="{{ $input }}">
            <option value="" disabled @selected($selCategory === null)>{{ __('expenses.form.select') }}</option>
    ...
    <div>
        <label for="category_custom" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.category_custom') }}</label>
        <input type="text" id="category_custom" name="category_custom" value="{{ old('category_custom', $expense->category_custom) }}" class="{{ $input }}">
    </div>

    <div>
        <label for="amount" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.gross_amount') }}<span class="text-red-600"> *</span></label>
        <input type="number" step="0.01" min="0" id="amount" name="amount" value="{{ $amount }}" required class="{{ $input }}">
        @error('amount')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
    </div>
    <div class="grid grid-cols-2 gap-2">
        <div>
            <label for="currency" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.currency') }}</label>
        ...
        <div>
            <label for="tax_rate" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.vat') }}</label>
            <input type="number" min="0" max="100" id="tax_rate" name="tax_rate" value="{{ old('tax_rate', $expense->tax_rate ?? 19) }}" class="{{ $input }}">
        </div>
    </div>

    <div>
        <label for="payment_status" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.payment_status') }}</label>
    ...
    <div>
        <label for="paid_on" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.paid_on') }}</label>
        <input type="date" id="paid_on" name="paid_on" value="{{ old('paid_on', $expense->paid_on?->format('Y-m-d')) }}" class="{{ $input }}">
    </div>

    <div>
        <label for="customer_id" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.customer') }}</label>
    ...
    <div>
        <label for="project_id" class="block text-sm font-medium text-gray-700">{{ __('expenses.form.project') }}</label>
    ...
    <div class="sm:col-span-2">
        <label class="inline-flex items-center gap-2 text-sm text-gray-700">
            ...
            {{ __('expenses.form.billable') }}
        </label>
    </div>

    <div class="sm:col-span-2">
        <x-tag-input name="labels" :value="old('labels', $expense->labels ?? [])" :label="__('expenses.form.labels')" />
    </div>
</div>
