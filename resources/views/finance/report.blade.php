<x-layouts.app title="Finance report">
    ...
    <div class="flex flex-wrap items-end justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Income vs expenses for {{ $from }} → {{ $to }} (base currency).</p>
        </div>
        <form method="GET" class="flex items-end gap-2">
            ...
            <button type="submit" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Apply</button>
        </form>
    </div>
    ...
    <div class="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-5">
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <span class="text-xs uppercase tracking-wide text-gray-400">Income</span>
            <div class="mt-1 text-lg font-semibold text-gray-900">{{ $eur($summary['income']) }}</div>
        </div>
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <span class="text-xs uppercase tracking-wide text-gray-400">Expenses</span>
            <div class="mt-1 text-lg font-semibold text-gray-900">{{ $eur($summary['expenses']) }}</div>
        </div>
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <span class="text-xs uppercase tracking-wide text-gray-400">Profit</span>
            <div class="mt-1 text-lg font-semibold {{ $summary['profit'] >= 0 ? 'text-green-700' : 'text-red-700' }}">{{ $eur($summary['profit']) }}</div>
        </div>
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <span class="text-xs uppercase tracking-wide text-gray-400">Outstanding</span>
            <div class="mt-1 text-lg font-semibold text-gray-900">{{ $eur($summary['outstanding']) }}</div>
        </div>
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <span class="text-xs uppercase tracking-wide text-gray-400">Unbilled work</span>
    ...
    <div class="mt-8 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <h2 class="text-sm font-semibold text-gray-900">Monthly income vs expenses</h2>
        ...
        <p class="mt-4 text-xs text-gray-400"><span class="inline-block h-2 w-2 rounded bg-green-500"></span> income · <span class="inline-block h-2 w-2 rounded bg-gray-400"></span> expenses</p>
    </div>

    <div class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
        {{-- Per customer --}}
        <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            <h2 class="border-b border-gray-100 px-4 py-3 text-sm font-semibold text-gray-900">Profit by customer</h2>
            @if (empty($perCustomer))
                <p class="px-4 py-6 text-center text-sm text-gray-500">No data.</p>
            @else
                <table class="min-w-full divide-y divide-gray-100 text-sm">
                    <thead class="bg-gray-50 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                        <tr><th class="px-4 py-2">Customer</th><th class="px-4 py-2 text-right">Revenue</th><th class="px-4 py-2 text-right">Expenses</th><th class="px-4 py-2 text-right">Profit</th></tr>
                    </thead>
        ...
        {{-- By category --}}
        <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            <h2 class="border-b border-gray-100 px-4 py-3 text-sm font-semibold text-gray-900">Expenses by category</h2>
            @if (empty($byCategory))
                <p class="px-4 py-6 text-center text-sm text-gray-500">No expenses.</p>
            @else
