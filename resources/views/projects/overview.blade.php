<x-layouts.app title="Projects">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">{{ __('projects.overview_heading') }}</h1>
            <p class="mt-1 text-sm text-gray-600">{{ __('projects.overview_subtitle') }}</p>
        </div>
        <a href="{{ route('projects.create') }}"
            ...
            {{ __('projects.overview_new_project') }}
        </a>
    </div>
        ...
        @else
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                    <tr>
                        <th scope="col" class="px-4 py-3"><x-sortable-header column="name" :label="__('projects.col_project')" :sort="$sort" :dir="$dir" /></th>
                        <th scope="col" class="px-4 py-3">{{ __('projects.col_customer') }}</th>
                        <th scope="col" class="px-4 py-3"><x-sortable-header column="type" :label="__('projects.col_type')" :sort="$sort" :dir="$dir" /></th>
                        <th scope="col" class="px-4 py-3"><x-sortable-header column="status" :label="__('projects.col_status')" :sort="$sort" :dir="$dir" /></th>
                        <th scope="col" class="px-4 py-3">{{ __('projects.col_tags') }}</th>
                    </tr>
                </thead>
