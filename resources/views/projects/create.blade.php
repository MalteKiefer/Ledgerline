<x-layouts.app title="New project">
    <p class="text-sm text-gray-500">
        @if ($lockedCustomer)
            ...
            <span aria-hidden="true">/</span> {{ __('projects.create_projects_crumb') }}
        @else
            <a href="{{ route('projects.overview') }}" class="hover:underline">{{ __('projects.create_projects_crumb') }}</a>
        @endif
    </p>
    <h1 class="mt-1 text-2xl font-semibold text-gray-900">{{ __('projects.create_heading') }}</h1>
    ...
    <form method="POST" action="{{ route('projects.store') }}"
        ...
        <div class="mt-6 flex items-center gap-3">
            <button type="submit"
                ...
                {{ __('projects.create_submit') }}
            </button>
            <a href="{{ $lockedCustomer ? route('customers.show', $lockedCustomer) : route('projects.overview') }}"
                class="text-sm text-gray-600 hover:text-gray-900">{{ __('projects.create_cancel') }}</a>
        </div>
    </form>
</x-layouts.app>
