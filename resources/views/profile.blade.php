<x-layouts.app :title="__('pages.profile.title')">
    ...
    {{-- Security & data --}}
    <div class="mt-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-6" x-data="{ del: false }">
        <h2 class="text-base font-semibold text-gray-900">{{ __('account.security_heading') }}</h2>

        <div class="mt-4 border-t border-gray-100 pt-4">
            <dt class="text-sm font-medium text-gray-500">{{ __('account.last_login') }}</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ $user->last_login_at?->format('Y-m-d H:i') ?: __('account.never') }}</dd>
        </div>

        {{-- Active sessions --}}
        <div class="mt-6 border-t border-gray-100 pt-4">
            <h3 class="text-sm font-medium text-gray-700">{{ __('account.sessions_heading') }}</h3>
            <p class="mt-1 text-xs text-gray-500">{{ __('account.sessions_hint') }}</p>
            <ul class="mt-3 divide-y divide-gray-100">
                @forelse ($sessions as $s)
                    <li class="flex items-center justify-between gap-3 py-2">
                        <div class="min-w-0">
                            <p class="truncate text-sm text-gray-900">{{ $s['agent'] ?: __('account.sessions_unknown') }}</p>
                            <p class="text-xs text-gray-500">
                                {{ $s['ip'] ?: '—' }} · {{ \Illuminate\Support\Carbon::createFromTimestamp($s['last_activity'])->diffForHumans() }}
                                @if ($s['current']) · <span class="font-medium text-gray-700">{{ __('account.sessions_current') }}</span>@endif
                            </p>
                        </div>
                        @unless ($s['current'])
                            <form method="POST" action="{{ route('account.sessions.revoke', $s['id']) }}" class="shrink-0">
                                @csrf @method('DELETE')
                                <button type="submit" class="inline-flex min-h-11 items-center rounded-md border border-gray-300 px-3 text-sm font-medium text-gray-700 hover:bg-gray-50">{{ __('account.sessions_revoke') }}</button>
                            </form>
                        @endunless
                    </li>
                @empty
                    <li class="py-2 text-sm text-gray-500">{{ __('account.sessions_none') }}</li>
                @endforelse
            </ul>
        </div>
        ...
        {{-- Export --}}
        <div class="mt-6 border-t border-gray-100 pt-4">
            <h3 class="text-sm font-medium text-gray-700">{{ __('account.export_heading') }}</h3>
            <p class="mt-1 text-xs text-gray-500">{{ __('account.export_hint') }}</p>
            <x-button :href="route('account.export')" icon="arrow-down-tray" class="mt-3">{{ __('account.export_button') }}</x-button>
        </div>

        {{-- Delete account --}}
        <div class="mt-6 border-t border-gray-100 pt-4">
            <h3 class="text-sm font-medium text-red-700">{{ __('account.delete_heading') }}</h3>
            <p class="mt-1 text-xs text-gray-500">{{ __('account.delete_hint') }}</p>
        ...
        <div x-show="del" x-cloak class="fixed inset-0 z-[80] flex items-start justify-center overflow-y-auto p-4"
            ...
            <div class="relative my-16 w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
                <h3 class="text-base font-semibold text-gray-900">{{ __('account.delete_modal_title') }}</h3>
                <p class="mt-2 text-sm text-gray-600">{{ __('account.delete_modal_warning') }}</p>
                <form method="POST" action="{{ route('account.destroy') }}" class="mt-4 space-y-3">
                    @csrf @method('DELETE')
                    <label class="block text-sm text-gray-700">{{ __('account.delete_confirm_label') }}
