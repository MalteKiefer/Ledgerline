 ...
 * BLOBS: it stores/streams ciphertext bytes at "files/{blob}" and keeps a blob
 * ownership ledger (file_blobs) for quota + access control. It cannot read a
 * blob's name, contents, or which manifest row references it.
 */
class FileController extends Controller
{
    ...
    public function reconcile(Request $request): JsonResponse
    {
        ...
        $grace = Carbon::now()->subHours((int) config('files.blob_orphan_grace_hours', 24));
        $disk = $this->disk();
    ...
    public function raw(string $blob): StreamedResponse
    {
        ...
        abort_unless(FileBlob::where('blob', $blob)->where('user_id', auth()->id())->exists(), 404);
        $path = 'files/'.$blob;
    ...
    public function deleteBlob(string $blob): JsonResponse
    {
        ...
        $row = FileBlob::where('blob', $blob)->first();
        if ($row === null) {
