...
/**
 * Stores the workspace encryption vault. Every value handled here is opaque
 * ciphertext or a public KDF parameter — the passphrase, recovery code and
 * vault key exist only in the browser and are never sent to the server.
 */
class VaultController extends Controller
{
    /**
     * Whether the vault is set up, and the material the browser needs to derive
     * and unwrap the vault key after the user types the passphrase.
     */
    ...
    /**
     * First-time setup: persist the wrapped vault key and KDF parameters.
     * Refuses to overwrite an existing vault (that is a passphrase change → rotate).
     */
    public function store(Request $request): JsonResponse
    {
        ...
        Vault::create($this->rules($request, withRecovery: true));
