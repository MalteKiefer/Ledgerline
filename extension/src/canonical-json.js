// Canonical JSON (Store v3, spec §5.2) — faithful port of
// resources/js/shared/canonical-json.js, byte-identical across JS/Swift/Go/Kotlin.
//
// Rules:
...
//   - Strings emitted AS-IS (no NFC normalisation), minimal JSON escaping, UTF-8.
...
//
// The extension writes the `passwords` and `bookmarks` module stores, which the
// web client seals canonically — this must stay byte-for-byte identical to the
// web canonical-json.js (see padme-cross-boundary tests).
...
function encodeString(s) {
    // Minimal JSON escaping, no normalisation. JSON.stringify emits exactly the
