 ...
 * (reset detection), the highest UID fetched so far, and CONDSTORE HIGHMODSEQ
 * where the server supports it. No user_id column: ownership flows through the
 * parent account.
 *
 ...
 * ledger's (user_id, content_hash) dedup in MaildirIngestor — resumability does
 * not depend on these columns. Kept for a future explicit server-side cursor.
 *
 * Eloquent has no native composite-primary-key support, so save()'s
 * update/select paths are pointed at both columns via the two overrides below.
 *
