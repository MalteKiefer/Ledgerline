...
/**
 * Crop a detected face out of an image into a small square JPEG. Boxes arrive
 * normalised (0..1); a little padding is added around the face. Uses raw Imagick
 * (already in the image) so the crop is independent of the thumbnail pipeline.
 */
class FaceCropper
{
    private const SIZE = 256;

    private const PAD = 0.2;
    ...
    public function crop(string $path, array $box): ?string
    {
        ...
        try {
            ...
            $img->cropThumbnailImage(self::SIZE, self::SIZE); // square cover
