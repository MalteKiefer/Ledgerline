 ...
 * mechanism that keeps the plaintext password OUT of the generated config file
 * on disk: mbsync shells this command at auth time and reads the password
 * directly from its stdout pipe. The plaintext therefore exists only
 * transiently, in this short-lived process's memory + the pipe — never at rest.
 *
 * Deliberately minimal and quiet: no Artisan banners, no logging of the account
 * or its password, exit 1 with empty stdout if the account cannot be found (so a
 * broken sync fails loudly with an auth error rather than authenticating with an
 * empty password).
 */
final class PrintMailAccountPassword extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Raw stdout write — no Artisan output decoration.
