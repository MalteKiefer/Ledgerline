...
final readonly class UpdateProjectTime
{
    ...
    public function handle(UpdateTimeData $data): TimeEntryView
    {
        ...
        $current = $this->work->timeEntry($data->projectId, $data->timeEntryUuid);
        if ($current->invoiceTargetReference !== null) {
            throw new InvalidProjectAction('time_entry_invoiced');
        }

        $rate = $data->hourlyRate;
        if ($rate === null && $current->hourlyRateMinor !== null && $current->currency === $data->currency) {
            $rate = Money::fromMinor($current->hourlyRateMinor, $current->currency);
        }
        $rate ??= $this->rates->frozenRate($data->projectId->ownerId, $project->partnerReference, $data->currency);
        if ($data->billable && $rate === null) {
            throw new InvalidProjectAction('hourly_rate_required');
        }

        return $this->work->updateTime($data, $rate);
    }
}
