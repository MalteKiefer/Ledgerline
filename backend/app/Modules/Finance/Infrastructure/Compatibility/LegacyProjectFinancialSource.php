...
final class LegacyProjectFinancialSource implements ProjectFinancialSource
{
    ...
    {
        $p = ProjectRecord::query()->withoutGlobalScopes()->where('user_id', $ownerId)->where('uuid', $projectId->uuid)->where('source_type', 'legacy_project')->firstOrFail();
        ...
        foreach (BankTransaction::query()->withoutGlobalScopes()->where('user_id', $ownerId)->where('finance_project_id', $legacy)->whereNull('deleted_at')->get() as $t) {
            ...
            $money = Money::fromDecimal((string) $t->amount, 'EUR');
            ...
            $rows[] = new ProjectFinancialSourceRow('finance-receipt:'.$r->id, -abs($money->minor()), $money->currency(), new DateTimeImmutable($r->date->format('Y-m-d')), $refs);
        }
