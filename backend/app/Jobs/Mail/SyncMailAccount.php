...
/**
 * PRODUCER of the mail-archive sync pipeline: fetch one account's mailbox, then
 * fan the fetched Maildir out to chunked ingest workers.
 *
 * Flow (per account):
 *   1. status = syncing.
 ...
 *      account errored with a redacted message and STOP — no ingest.
 *   4. On Success, PAGE the Maildir (folders × their cur/+new/ files) into
 *      IngestMailChunk jobs of a bounded size and dispatch them as a batch — a
 *      100k-message mailbox becomes ~1k chunk jobs, not 100k.
...
class SyncMailAccount implements ShouldQueue
{
    ...
    public function handle(MbsyncRunner $runner): void
    {
        ...
        if (! $result->ok) {
            // Failed/HostRejected: the runner already recorded this (idempotent).
            // Unavailable: the runner left the account untouched, so we record the
            // error here. Either way: no ingest.
        ...
        // capture $this (the job): it captures only the account id + a static settler.
     ...
     * subdir; its ledger name is the path relative to the root (e.g. "INBOX",
     ...
     * skipping dotfiles (incl. `.quarantine/`) and non-files.
     *
     * @return list<string>
     */
