<?php

declare(strict_types=1);

return [
    'calendar_section' => 'Kalender',
    'calendar_desc' => 'Wochenstart, Kalenderwochen und die Standarddauer neuer Termine.',
    'calendar_heading' => 'Kalender-Einstellungen',
    'calendar_display_heading' => 'Anzeige & Verhalten',
    'calendar_display_hint' => 'Wie der Kalender angezeigt wird und welche Vorgaben neue Termine haben.',
    'calendar_week_start' => 'Woche beginnt am',
    'calendar_week_start_monday' => 'Montag',
    'calendar_week_start_sunday' => 'Sonntag',
    'calendar_week_numbers' => 'Kalenderwochen anzeigen',
    'calendar_timezone' => 'Zeitzone',
    'calendar_timezone_auto' => 'Automatisch (diesem Browser folgen)',
    'calendar_timezone_hint' => 'Kalender auf eine feste Zeitzone festlegen oder automatisch die Zone des jeweiligen Geräts verwenden.',
    'calendar_default_event_minutes' => 'Standard-Termindauer (Minuten)',
    'calendar_default_event_minutes_hint' => 'Vorausgefüllte Länge beim Anlegen eines neuen Termins mit Uhrzeit.',
    'calendar_contacts_heading' => 'Kalender aus Kontakten',
    'calendar_contacts_hint' => 'Automatisch schreibgeschützte Kalender aus den Datumsangaben deiner Kontakte erzeugen (jährlich, ganztägig).',
    'calendar_birthdays_enabled' => 'Geburtstagskalender anzeigen (aus Kontakt-Geburtstagen)',
    'calendar_anniversaries_enabled' => 'Jahrestagskalender anzeigen (aus Kontakt-Jahrestagen)',
    'calendar_holidays_heading' => 'Feiertage',
    'calendar_holidays_hint' => 'Einen schreibgeschützten Feiertagskalender für die gewählten Länder anzeigen.',
    'gallery_desc' => 'Reise-Gruppierung und ein Metadaten-Rescan für die Fotogalerie.',
    'gallery_heading' => 'Galerie-Einstellungen',
    'gallery_section' => 'Galerie',
    'heading' => 'Einstellungen',
    'index_title' => 'Einstellungen',
    'personal_heading' => 'Meine Einstellungen',
    'personal_subheading' => 'Einstellungen, die nur für dein Konto gelten.',
    'admin_heading' => 'Administration',
    'admin_note' => 'Diese Einstellungen gelten für den gesamten Workspace und betreffen alle Nutzer.',
    'contacts_section' => 'Kontakte',
    'contacts_desc' => 'CardDAV-Sync — Kontakte mit Handy und Desktop-Clients verbinden.',
    'sync_section' => 'Kontakte- & Kalender-Sync',
    'sync_block_hint' => 'Ein Login synchronisiert Kontakte und Kalender mit Handy und Desktop-Clients. Das Passwort wird nur einmal bei der Erstellung angezeigt.',
    'downloads_section' => 'Downloads & Exporte',
    'downloads_desc' => 'Maximale Export-Zip-Größe und Benachrichtigungen bei fertigen Exporten.',
    'reminders_section' => 'Erinnerungen',
    'reminders_desc' => 'Welche Kanäle bei neuen Erinnerungen vorausgewählt sind.',
    'reminders_heading' => 'Erinnerungs-Standard',
    'reminders_subheading' => 'Kanäle, die vorausgewählt sind, wenn du eine Aufgaben- oder Termin-Erinnerung anlegst. Die Kanäle selbst konfiguriert ein Administrator.',
    'reminders_saved' => 'Erinnerungs-Standard gespeichert.',
    'files_limits_heading' => 'Globale Datei-Limits',
    'files_limits_hint' => 'Nur Admin. Feld leer lassen = eingebauter Standard.',
    'files_quota' => 'Speicher-Quota pro Nutzer (MB, 0 = unbegrenzt)',
    'files_max_upload' => 'Max. Upload-Größe (MB)',
    'files_trash_retention' => 'Papierkorb-Aufbewahrung (Tage)',
    'files_archive_entries' => 'Archiv: max. Dateien',
    'files_archive_mb' => 'Archiv: max. entpackt (MB)',
    'files_orphan_grace' => 'Verwaiste Blobs: Karenz (Std.)',
    'files_default_ph' => 'Standard: :n',
    'exiftool_path' => 'exiftool-Pfad',
    'exiftool_path_hint' => 'Binary für HEIC-/Live-Photo-Metadaten.',
    'ml_heading' => 'ML & Gesichtserkennung',
    'ml_hint' => 'Benötigt den ML-Dienst. Feld auf Standard/leer = eingebauter Wert.',
    'ml_enabled' => 'Bild-ML (CLIP / Duplikate)',
    'face_enabled' => 'Gesichtserkennung (Personen)',
    'ml_default' => 'Standard',
    'ml_on' => 'An',
    'ml_off' => 'Aus',
    'ml_advanced' => 'Erweitert',
    'ml_url' => 'ML-Dienst-URL',
    'ml_clip_model' => 'CLIP-Modell',
    'face_model' => 'Gesichts-Modell',
    'duplicate_threshold' => 'Duplikat-Ähnlichkeit (0–1)',
    'phash_max_distance' => 'pHash max. Distanz',
    'face_min_score' => 'Gesicht Min-Score (0–1)',
    'face_min_size' => 'Gesicht Min-Größe (px)',
    'face_cluster_threshold' => 'Gesicht-Cluster-Ähnlichkeit (0–1)',
    'face_min_per_person' => 'Min. Gesichter pro Person',
    'geocode_interval_ms' => 'Geocode-Intervall (ms)',
    'backup_decrypt' => 'Entschlüsseln',
    'backup_decrypt_hint' => 'Dieses verschlüsselte Backup holen und mit der Passphrase in ein herunterladbares Archiv entschlüsseln. In der App wird nichts verändert.',
    'backup_decrypt_download' => 'Entschlüsseln & herunterladen',
    'backup_decrypt_failed' => 'Falsche Passphrase, oder das Archiv konnte nicht entschlüsselt werden.',
    'files_reindex_queued' => 'Dateien werden neu indiziert…',
    'files_reindex_heading' => 'Volltext-Suchindex',
    'files_reindex_hint' => 'Text (OCR) aus allen Dateien neu extrahieren, z. B. nach Dateien von vor der Suche.',
    'files_reindex_action' => 'Dateien neu indizieren',
    'calendar_refresh_heading' => 'Abonnierte Kalender',
    'calendar_refresh_hint' => 'Alle abonnierten ICS-Kalender jetzt aktualisieren, statt auf den periodischen Sync zu warten.',
    'calendar_refresh_action' => 'Abos aktualisieren',
    'system_section' => 'System',
    'system_desc' => 'Geplante Wartungsaufgaben und ihr letzter Lauf.',
    'system_cron_heading' => 'Geplante Aufgaben',
    'system_cron_hint' => 'Hintergrund-Wartung durch den Scheduler. Läuft eine Aufgabe nie, ist evtl. der Cron-/Scheduler-Container aus.',
    'system_task' => 'Aufgabe',
    'system_schedule' => 'Zeitplan',
    'system_last_run' => 'Letzter Lauf',
    'system_never' => 'Nie',
    'files_section' => 'Dateien',
    'files_desc' => 'Wie viele frühere Versionen einer Datei aufbewahrt werden.',
    'files_webdav_heading' => 'WebDAV-Zugriff',
    'files_webdav_hint' => 'Binde deine Dateien als Netzlaufwerk mit dem DAV-Login ein (wie Kontakte/Kalender): macOS Finder „Mit Server verbinden“, Windows Explorer oder eine WebDAV-App.',
    'files_webdav_manage' => 'DAV-Zugangsdaten verwalten',
    'files_max_versions' => 'Aufbewahrte Versionen pro Datei',
    'files_max_versions_hint' => 'Wenn sich eine Datei ändert, werden bis zu so viele frühere Versionen behalten (1–10).',
    'files_saved' => 'Datei-Einstellungen gespeichert.',
    'batch_done' => 'Fertig',
    'batch_gone' => 'Dieser Lauf ist abgeschlossen oder abgelaufen.',
    'batch_heading' => 'Verarbeitungsfortschritt',
    'count_images' => 'Fotos',
    'count_motion' => 'Motion-Fotos',
    'count_total' => 'Elemente gesamt',
    'count_videos' => 'Videos',
    'ffmpeg_missing' => 'nicht gefunden',
    'ffmpeg_ok' => 'verfügbar',
    'ffmpeg_path' => 'ffmpeg-Pfad',
    'ffmpeg_path_hint' => 'Pfad zum ffmpeg-Binary für Video-Thumbnails. Leer lassen, um GALLERY_FFMPEG_PATH oder den System-PATH zu nutzen.',
    'ffmpeg_resolved' => 'Aktuell verwendet',
    'filename_template' => 'Dateinamen-Vorlage',
    'geocode_grid_km' => 'Standort-Raster (km)',
    'geocode_grid_km_hint' => 'Fotos in diesem Umkreis teilen sich eine Ortsabfrage. Größer spart Anfragen, macht die Adresse aber gröber. 0 = jede Koordinate einzeln abfragen.',
    'general_heading' => 'Allgemein (Uploads)',
    'general_hint' => 'Einstellungen, die für jeden Upload gelten.',
    'filename_template_hint' => 'Wird beim Einlesen auf die Anzeigenamen angewendet. Platzhalter: {{y}} {{yy}} {{MM}} {{dd}} {{HH}} {{mm}} {{ss}} {{filename}} {{ext}}. Die ursprüngliche Endung bleibt immer erhalten. Leer lassen, um die Originalnamen zu behalten.',
    'map_zoom' => 'Karten-Zoomstufe',
    'max_upload_mb' => 'Max. Upload-Größe (MB)',
    'max_upload_mb_hint' => 'Server-Upload-Limits (PHP/Proxy) können zusätzlich greifen.',
    'map_zoom_hint' => 'Zoom (1–19) für die Kartenelemente, z. B. die Detail-Minikarte.',
    'job_scope_all' => 'Alle Elemente (:count)',
    'job_scope_hint' => 'Für die ganze Bibliothek ausführen oder nur die neuesten Elemente?',
    'job_scope_missing' => 'Nur wo es fehlt (z. B. Thumbnails, Metadaten)',
    'job_scope_recent' => 'Nur die neuesten',
    'job_scope_run' => 'Ausführen',
    'maintenance_heading' => 'Wartung',
    'maintenance_hint' => ':count Element(e) (Fotos und Videos). Jeden Hintergrund-Job einzeln ausführen oder alle auf einmal.',
    'photos_heading' => 'Fotos',
    'queue_connection' => 'Verbindung',
    'queue_failed' => 'Fehlgeschlagene Jobs',
    'queue_heading' => 'Warteschlangen-Status',
    'queue_hint' => 'Hintergrund-Jobs, die auf Ausführung warten. Aktualisiert sich automatisch.',
    'queue_pending' => 'Wartende Jobs',
    'queue_pending_unsupported' => 'Von diesem Warteschlangen-Treiber nicht gemeldet',
    'queue_workers_note' => 'Laufende Worker und eine Restzeit-Schätzung werden nicht in der App erfasst. Bei einer verwalteten Warteschlange (z. B. Laravel Cloud) stehen sie im Dashboard des Anbieters.',
    'regenerate' => 'Thumbnails neu erzeugen',
    'rename' => 'Dateinamen-Vorlage anwenden',
    'run_all_jobs' => 'Alle Jobs ausführen',
    'detect_duplicates' => 'Nach Duplikaten suchen',
    'detect_faces' => 'Nach Gesichtern suchen',
    'rescan' => 'Metadaten neu einlesen',
    'save' => 'Speichern',

    'subheading' => 'Verwalten Sie die Konfiguration Ihres Arbeitsbereichs.',
    'trip_gap_days' => 'Reise-Abstand (Tage)',
    'trip_radius_km' => 'Reise-Radius (km)',
    'trips_hint' => 'Eine neue Reise beginnt, wenn Fotos weiter als diese Tage auseinander oder weiter als dieser Radius entfernt sind.',
    'video_frame' => 'Video-Thumbnail-Sekunde',
    'video_frame_hint' => 'Die Sekunde im Video, die als Thumbnail verwendet wird.',
    'video_heading' => 'Videos & Uploads',
    'video_hint' => 'Video-Thumbnails brauchen ffmpeg. Videos werden inline abgespielt und erst beim Play geladen.',
    'notifications_section' => 'Benachrichtigungen',
    'notifications_desc' => 'Mailserver, NTFY und Webhook — die Kanäle für Backup-Meldungen.',
    'notifications_heading' => 'Benachrichtigungs-Kanäle',
    'notify_secret_keep_hint' => 'Leer lassen, um den gespeicherten Wert zu behalten.',
    'notify_mail_heading' => 'Mailserver (SMTP)',
    'notify_mail_hint' => 'Verschlüsselt gespeichert, aber im Klartext zum Versand genutzt.',
    'smtp_host' => 'SMTP-Host',
    'smtp_port' => 'Port',
    'smtp_encryption' => 'Verschlüsselung',
    'smtp_username' => 'Benutzername',
    'smtp_password' => 'Passwort',
    'smtp_from_address' => 'Absender-Adresse',
    'smtp_from_name' => 'Absender-Name',
    'notify_ntfy_heading' => 'NTFY',
    'ntfy_url' => 'Server-URL',
    'ntfy_topic' => 'Topic',
    'ntfy_token' => 'Access-Token (optional)',
    'notify_webhook_heading' => 'Webhook',
    'webhook_url' => 'URL',
    'webhook_secret' => 'Signatur-Secret (optional)',
    'notify_test_heading' => 'Test senden',
    'notify_test_hint' => 'Sendet eine Testnachricht mit den oben gespeicherten Einstellungen — bitte erst speichern.',
    'notify_test_send' => ':channel testen',
    'backup_section' => 'Backup',
    'backup_desc' => 'Zeitgesteuerte Sicherungen von Datenbank, Dateien und Galerie nach S3, B2, SFTP oder WebDAV.',
    'backup_heading' => 'Backup',
    'backup_destinations_heading' => 'Ziele',
    'backup_jobs_heading' => 'Aufgaben',
    'backup_runs_heading' => 'Letzte Läufe',
    'backup_add_destination' => 'Ziel hinzufügen',
    'backup_add_job' => 'Aufgabe hinzufügen',
    'backup_no_destinations' => 'Noch keine Ziele — lege eins an, um Aufgaben zu konfigurieren.',
    'backup_no_jobs' => 'Noch keine Backup-Aufgaben.',
    'backup_no_runs' => 'Noch keine Läufe.',
    'backup_connect_failed' => 'Verbindungstest fehlgeschlagen.',
    'backup_name' => 'Name',
    'backup_driver' => 'Treiber',
    'backup_source' => 'Quelle',
    'backup_destination' => 'Ziel',
    'backup_cron' => 'Zeitplan (Cron)',
    'backup_cron_hint' => 'Cron mit 5 Feldern, z. B. „0 */3 * * *" = alle 3 Stunden.',
    'backup_cron_invalid' => 'Kein gültiger Cron-Ausdruck.',
    'backup_retention' => 'Versionen behalten',
    'backup_encrypt' => 'Archiv verschlüsseln',
    'backup_passphrase' => 'Passphrase',
    'backup_passphrase_required' => 'Bei aktiver Verschlüsselung ist eine Passphrase erforderlich.',
    'backup_db_encrypt_required' => 'Ein Datenbank-Backup muss verschlüsselt sein (enthält andere Module im Klartext). Verschlüsselung aktivieren + Passphrase setzen.',
    'backup_notify' => 'Benachrichtigen via',
    'backup_enabled' => 'Aktiv',
    'backup_run_now' => 'Jetzt sichern',
    'backup_status' => 'Status',
    'backup_started' => 'Gestartet',
    'backup_size' => 'Größe',
    'backup_bucket' => 'Bucket',
    'backup_region' => 'Region',
    'backup_key' => 'Access Key',
    'backup_secret' => 'Secret Key',
    'backup_endpoint' => 'Endpoint (B2 / S3-kompatibel)',
    'backup_use_path_style' => 'Path-Style-Endpoint',
    'backup_host' => 'Host',
    'backup_port' => 'Port',
    'backup_username' => 'Benutzername',
    'backup_password' => 'Passwort',
    'backup_base_uri' => 'Basis-URI',
    'backup_path' => 'Pfad / Präfix',
    'backup_save' => 'Speichern',
    'backup_delete_confirm' => 'Dies löschen? Kann nicht rückgängig gemacht werden.',
    'backup_never_run' => 'nie',
    'backup_source_database' => 'Datenbank',
    'backup_source_files' => 'Dateien',
    'backup_source_gallery' => 'Galerie',
    'backup_test' => 'Verbindung testen',
    'backup_stat_runs' => 'Läufe',
    'backup_stat_success_rate' => 'Erfolgsquote',
    'backup_stat_last_duration' => 'Letzte Dauer',
    'backup_stat_avg_duration' => 'Ø Dauer',
    'backup_stat_last_size' => 'Letzte Größe',
    'backup_stat_total_size' => 'Gesamt gespeichert',
    'backup_stat_last_run' => 'Letzter Lauf',
    'backup_stat_next_run' => 'Nächster Lauf',
    'backup_no_log' => 'Kein Protokoll für diesen Lauf.',
    'backup_path_hint' => 'Relativ zum Login-Verzeichnis. Leer lassen für Standard (viele SFTP-Hosts erlauben kein Schreiben im Server-Root).',
    'backup_notify_desktop' => 'Desktop / Browser',
    'backup_queued_short' => 'Eingereiht…',
    'backup_download' => 'Herunterladen',
    'backup_cancel' => 'Abbrechen',
    'backup_cancelling' => 'Wird abgebrochen…',
    'backup_force_stop' => 'Sofort stoppen',
    'backup_cron_tz' => 'Zeiten gelten in der Zeitzone :tz.',

    'paperless_section' => 'Paperless',
    'paperless_desc' => 'Paperless-ngx-Instanz verbinden, um Dokumente zu senden und Tags, Typen und Korrespondenten zu übernehmen.',
    'paperless_heading' => 'Paperless',
    'paperless_enabled' => 'Paperless-Integration aktivieren',
    'paperless_url' => 'Instanz-URL',
    'paperless_token' => 'API-Token',
    'paperless_token_hint' => 'Token in Paperless unter Einstellungen → API-Token anlegen. Leer lassen, um den gespeicherten zu behalten.',
    'paperless_test' => 'Verbindung testen',
    'paperless_testing' => 'Teste…',
    'paperless_test_ok' => 'Verbindung OK.',
    'paperless_test_ok_detail' => 'Verbindung OK — :tags Tag(s), :types Dokumenttyp(en), :correspondents Korrespondent(en).',
    'paperless_test_failed' => 'Verbindungstest fehlgeschlagen.',
    'paperless_test_missing' => 'Zuerst URL und API-Token eingeben.',
    'paperless_cache_heading' => 'Zwischengespeicherte Schnellauswahl',
    'paperless_sync_now' => 'Jetzt aktualisieren',
    'paperless_syncing' => 'Aktualisiere…',
    'paperless_tags' => 'Tags',
    'paperless_document_types' => 'Dokumenttypen',
    'paperless_correspondents' => 'Korrespondenten',
    'paperless_synced_at' => 'Zuletzt aktualisiert :time.',
    'paperless_never_synced' => 'Noch nicht aktualisiert.',
    'backup_mirror_hint' => 'Inkrementeller Spiegel: nur geänderte Dateien werden übertragen; Versionen, Aufbewahrung und Archiv-Verschlüsselung entfallen.',
    'backup_mode' => 'Backup-Modus',
    'backup_mode_mirror' => 'Inkrementeller Spiegel (nur Änderungen)',
    'backup_mode_archive' => 'Vollständiges Archiv (tar.gz)',
    'safe_url' => ':attribute muss eine http(s)-URL sein und darf nicht auf eine Link-local- oder Metadaten-Adresse zeigen.',
];
