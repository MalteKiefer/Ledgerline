<?php

declare(strict_types=1);

return [
    'title' => 'Passwords',
    'new' => 'New',
    'all' => 'All',
    'close' => 'Close',
    'refresh_icons' => 'Refresh icons',
    'icons_done' => 'Icons updated.',
    'export' => 'Export',
    'export_plaintext' => 'Plaintext export',
    'export_json' => 'Export JSON',
    'export_csv' => 'Export CSV',
    'export_encrypted' => 'Encrypted export',
    'export_warning' => 'Warning: plaintext exports contain ALL passwords, passkey private keys, and other secrets in clear text. Store the file securely and delete it immediately after use.',
    'export_confirm' => 'I understand — export unencrypted',
    'export_passphrase' => 'Export passphrase',
    'export_done' => 'File downloaded.',
    'export_failed' => 'Export failed.',
    'export_shared_note' => 'Shared-vault items are not included (personal vault only).',
    'reset' => 'Reset password manager',
    'reset_title' => 'Reset password manager',
    'reset_warning' => 'This permanently deletes ALL your passwords, passkeys and personal vaults, then creates one fresh empty vault. This cannot be undone — export first if you want a backup.',
    'reset_shared_note' => 'Shared vaults you manage are also deleted for everyone. Vaults shared WITH you (where you are a member) are kept.',
    'reset_confirm_prompt' => 'Type :word to confirm.',
    'reset_confirm_word' => 'RESET',
    'reset_button' => 'Delete everything and reset',
    'reset_done' => 'Password manager reset.',
    'reset_failed' => 'Reset failed.',
    'reset_partial' => 'Reset done, but some managed shared vaults could not be deleted.',
    'default_vault_name' => 'Private',
    'import' => 'Import',
    'import_intro' => 'Import from another manager. Everything is parsed in your browser and sealed locally — the file is never uploaded.',
    'import_format' => 'Format',
    'import_target_vault' => 'Import into vault',
    'import_folder_note' => 'Source folders become tags; every item is placed in the selected vault.',
    'import_running' => 'Importing…',
    'import_done' => 'Imported :count entries.',
    'import_failed' => 'Could not read that file. Check the format.',
    'import_hint' => 'Export an unencrypted file from the other app first. Delete it afterwards — it contains your passwords in clear text.',
    'fmt_auto' => 'Auto-detect',
    'fmt_bitwarden_json' => 'Bitwarden (.json)',
    'fmt_bitwarden_csv' => 'Bitwarden (.csv)',
    'fmt_onepassword' => '1Password (.csv)',
    'fmt_lastpass' => 'LastPass (.csv)',
    'fmt_keepass' => 'KeePass (.csv)',
    'fmt_generic' => 'Chrome / generic (.csv)',
    'search' => 'Search…',
    'empty' => 'Nothing here yet.',
    'pick_hint' => 'Pick an item or create a new one.',
    'title_placeholder' => 'Title',
    'favorite' => 'Favorite',
    'edit' => 'Edit',
    'save' => 'Save',
    'cancel' => 'Cancel',
    'delete' => 'Move to trash',
    'restore' => 'Restore',
    'trash' => 'Trash',
    'empty_trash' => 'Empty trash',
    'empty_trash_confirm' => 'Permanently delete everything in the trash?',
    'tfa_available' => 'This website offers two-factor authentication.',
    'tfa_add' => 'Add a one-time code',
    'tfa_how' => 'How to enable it',
    'selected' => 'selected',
    'select_all' => 'Select all',
    'bulk_purge_confirm' => 'Permanently delete the selected entries?',
    'delete_confirm' => 'Move this entry to the trash?',
    'updated' => 'updated',
    'copied' => 'Copied — clears shortly',
    'yes' => 'Yes',
    'no' => 'No',
    'wifi_scan' => 'Scan to join the network',
    'large_type' => 'Large type',
    'reveal' => 'Show',
    'hide' => 'Hide',
    // Password strength estimation
    'strength' => 'Strength',
    'strength_very_weak' => 'Very Weak',
    'strength_weak' => 'Weak',
    'strength_fair' => 'Fair',
    'strength_good' => 'Good',
    'strength_strong' => 'Strong',
    'crack_time' => 'Crack time',

    // Password health + breach check
    'health' => 'Health',
    'health_issues' => 'issues',
    'health_ok' => 'No issues found.',
    'check_breaches' => 'Check breaches',
    'checking' => 'Checking…',
    'issue_breached' => 'Breached',
    'issue_reused' => 'Reused',
    'issue_weak' => 'Weak',
    'issue_no2fa' => 'No 2FA',
    'issue_expiring' => 'Expiring',
    'card_expiring_warn' => 'This card is expired or expiring soon.',
    'breach_warn' => 'This password appeared in :count known breaches. Change it.',
    'reused_warn' => 'This password is reused on another entry.',
    'reused_where' => 'Also used in:',
    'weak_warn' => 'This password is weak.',
    'generate' => 'Generate password',
    'generate_apply' => 'Use password',
    'copy' => 'Copy',
    'gen_regen' => 'Regenerate',
    'gen_chars' => 'Characters',
    'gen_words' => 'Memorable words',
    'gen_similar' => 'Allow look-alike characters',
    'gen_wordcount' => 'Words',
    'gen_language' => 'Language',
    'gen_separator' => 'Separator',
    'gen_sep_space' => 'Space',
    'gen_sep_none' => 'None',
    'gen_capitalize' => 'Capitalize',
    'gen_number' => 'Add number',
    'length' => 'Length',
    'history' => 'History',
    'history_empty' => 'No previous versions.',
    'version_restore' => 'Restore',
    'versions_heading' => 'Version history',
    'no_change' => 'No visible change',

    // Shared vaults (read + write)
    'save_conflict' => 'Vault was updated elsewhere — reloading.',
    'save_failed' => 'Save failed. Please try again.',
    'shared_vaults' => 'Shared Vaults',
    'shared_vault_untitled' => 'Untitled Shared Vault',
    'role_read' => 'Viewer',
    'role_edit' => 'Editor',
    'role_manage' => 'Manager',
    'read_only_notice' => 'You have read-only access to this vault.',
    'new_shared_vault' => 'New shared vault',
    'new_shared_vault_name' => 'Vault name',
    'move_to' => 'Move to…',
    'move_denied' => "You can't move items into or out of a read-only vault.",
    'no_shared_yet' => 'No shared vaults yet.',

    // Vault sharing (manager)
    'share' => 'Share',
    'share_vault' => 'Share Vault',
    'recipient' => 'Recipient',
    'recipient_identifier' => 'Email or OIDC subject',
    'look_up' => 'Look up',
    'recipient_not_found' => 'Recipient not found or has no identity key registered.',
    'fingerprint' => 'Key fingerprint',
    'fingerprint_verify' => 'Verify this fingerprint with the recipient out-of-band before sharing.',
    'fingerprint_verified' => 'Verified',
    'fingerprint_changed_warn' => 'Warning: the stored fingerprint for this recipient has changed. This may indicate a key compromise. Share aborted.',
    'invite_sent' => 'Invitation sent.',
    'already_member' => 'This user is already a member of the vault.',
    'pending_invites' => 'Pending Invitations',
    'accept' => 'Accept',
    'invite_invalid' => 'Could not decrypt the invitation. The key may be invalid.',
    'role_viewer' => 'Viewer',
    'role_editor' => 'Editor',
    'role_manager' => 'Manager',

    // Vault member management + revocation
    'manage_members' => 'Manage members',
    'members' => 'Members',
    'member_status_pending' => 'Pending',
    'member_status_active' => 'Active',
    'remove_member' => 'Remove',
    'remove_member_confirm' => 'Remove this member? The vault key will be rotated — they will lose access immediately.',
    'rotating_keys' => 'Rotating keys…',
    'delete_vault' => 'Delete vault',
    'delete_vault_confirm' => 'Permanently delete this shared vault and all its contents? This cannot be undone.',
    'member_removed' => 'Member removed.',

    // Folders + tags + custom fields
    'folders' => 'Vaults',
    'new_folder' => 'New vault',
    'no_folder' => 'No vault',
    'folder' => 'Vault',
    'folder_name' => 'Vault name',
    'all_vaults' => 'All vaults',
    'filter_type' => 'Filter by type',
    'delete_folder_confirm' => 'Delete this vault? Its entries are moved to another vault.',
    'tags' => 'Tags',
    'tags_placeholder' => 'comma, separated',
    'custom_fields' => 'Custom fields',
    'custom_label' => 'Label',
    'custom_value' => 'Value',
    'custom_secret' => 'Hide value',
    'kind_text' => 'Text',
    'kind_secret' => 'Hidden',
    'kind_multiline' => 'Multiline',
    'kind_url' => 'URL',
    'add_field' => 'Add field',
    'add_url' => 'Add URL',
    'title_label' => 'Title',
    'custom_changed' => 'Custom fields',

    // Types
    'type_login' => 'Login',
    'type_password' => 'Password',
    'type_card' => 'Credit card',
    'type_wifi' => 'Wi-Fi',
    'type_license' => 'Software license',
    'type_server' => 'Server',
    'type_passkey' => 'Passkey',
    'type_identity' => 'Identity',
    'type_secure_note' => 'Secure note',

    // Passkey
    'passkey_ext_only' => 'Passkeys are created and used through the Ledgerline browser extension.',
    'passkey_created' => 'Created',
    'passkey_algo' => 'Algorithm',
    'passkey_attached' => 'Passkey',
    'passkey_remove' => 'Remove passkey',
    'passkey_remove_confirm' => 'Remove this passkey from the item?',
    'passkey_none' => 'No passkeys attached',

    // Fields
    'f_username' => 'Username',
    'f_password' => 'Password',
    'f_url' => 'URL',
    'f_urls' => 'URLs',
    'f_totp' => 'One-time code (TOTP)',
    'f_note' => 'Notes',
    'f_cardholder' => 'Cardholder',
    'f_number' => 'Card number',
    'f_brand' => 'Brand',
    'f_expiry' => 'Expiry (MM/YY)',
    'f_cvv' => 'CVV',
    'f_pin' => 'PIN',
    'f_ssid' => 'Network name (SSID)',
    'f_security' => 'Security',
    'f_hidden' => 'Hidden network',
    'f_product' => 'Product',
    'f_licensekey' => 'License key',
    'f_owner' => 'Owner',
    'f_email' => 'Email',
    'f_host' => 'Host / IP',
    'f_port' => 'Port',
    'f_rpId' => 'Website (domain)',
    'f_userName' => 'Account',
    'f_userDisplayName' => 'Display name',
    'f_firstName' => 'First name',
    'f_lastName' => 'Last name',
    'f_phone' => 'Phone',
    'f_company' => 'Company',
    'f_street' => 'Street',
    'f_city' => 'City',
    'f_state' => 'State / Province',
    'f_zip' => 'ZIP / Postal code',
    'f_country' => 'Country',
];
