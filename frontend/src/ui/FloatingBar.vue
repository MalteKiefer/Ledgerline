<template>
  ...
  <Teleport to="body">
    ...
    >
      <div
        v-if="show"
        class="fixed bottom-5 left-1/2 z-[1400] flex max-w-[calc(100vw-1.5rem)] -translate-x-1/2 items-center gap-2 overflow-x-auto rounded-2xl border border-[var(--ll-border)] bg-[var(--ll-elevated)] px-3 py-2 shadow-2xl"
        role="toolbar" :aria-label="label"
      >
        <span v-if="label" class="shrink-0 whitespace-nowrap px-1 text-sm font-medium">{{ label }}</span>
        <slot />
      </div>
    </Transition>
  </Teleport>
