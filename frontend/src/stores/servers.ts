...
export interface HostingPanel {
  ...
  details?: {
    ...
    extensions?: string[];
    disk_gb?: number;
    ips?: { address: string; mask: string; interface: string; public: string | null; main: boolean }[];
    databases?: { name: string; type: string; domain: string | null }[];
...
export interface ProbeResult {
  ...
  public_key: string | null;
  /** SHA256 of that key, to compare with what the host lists. */
...
function probeFrom(e: unknown): ProbeResult {
  ...
  return { ok: false, error: null, fingerprint: null, public_key: null, key_fingerprint: null, duration_ms: 0 };
}
