...
async function save() { saving.value = true; try { configured.value = (await api.put<{ configured: boolean }>('/api/v1/admin/virustotal', apiKey.value ? { api_key: apiKey.value } : {})).configured; apiKey.value = ''; success(t('common.saved')); } catch { error(t('common.error')); } finally { saving.value = false; } }
</script>
