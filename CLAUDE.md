...
- **Erledigt (Forts.):** `shared/markdown.js` (v1.500.11); `shared/zk-module.js` (v1.500.12); erste `components/*`-Dateien: toast-hub, crop-modal (v1.500.13); backup-runs, device-pairing (v1.500.14); paperless-settings, notification-bell (v1.500.15); eslint no-undef-Netz (v1.500.16); große: contacts (v1.500.17), passwords (v1.500.18) raus.
- **Nächste (groß):** passwords, files, gallery als eigene Dateien (eslint `no-undef` verifiziert je fehlende Imports). Dann Rest-Einfache (publicShare/fileShare/invoices/todos/notes/bookmarks) + Stores., dann Stores + einfache Komponenten, zuletzt gallery/files/passwords in Mixins.
