...
Rechnungen, Backup, Paperless**. Version **v1.502.0** (live https://home.kiefer-networks.de, `/up`=200).
...
- **Passkeys / WebAuthn-Authenticator** (v1.502.0):
  ...
  - **`rpIdAllowed(originHost, rpId)`** (`passkey.js`): pure, getestete Funktion — rpId muss gleich dem Origin-Host oder ein Dot-Boundary-Parent sein (parent→child only, nie reverse). Mirrors `hostsMatch()` aus background.js exakt (inkl. TLD-Gap ohne PSL). Vitest-getestet.
...
- **Origin aus trusted Content-Script-Kontext:** Die `origin`-Property im `passkey.create`/`passkey.get`-Message wird vom Content-Script auf `location.origin` gesetzt (ISOLATED-World, nicht page-controlled). Der MAIN-world Shim darf keinen eigenen `origin`-Wert übergeben — er übergibt `location.origin` als Parameter, aber das Content-Script überschreibt es auf die eigene `location.origin`. Verhindert WebAuthn-Origin-Binding-Bypass (RP kann nicht beliebige Origins vortäuschen).
...
- **Bare-TLD-Gap in `rpIdAllowed`:** `('example.com','com')→true` wegen `endsWith('.com')` ohne PSL-Lookup. Mirrors `hostsMatch()` in background.js exakt. Trade-off: kein PSL-Dependency (Binary-Größe, Update-Lag). Risiko niedrig: Browser selbst haben denselben Gap in nicht-PSL-fähigen Builds; RPs mit korrektem rpId nicht betroffen.
