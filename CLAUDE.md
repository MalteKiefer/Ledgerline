...
- **Erledigt (Forts.):** `shared/markdown.js` (v1.500.11); `shared/zk-module.js` (v1.500.12); erste `components/*`-Dateien: toast-hub, crop-modal (v1.500.13); backup-runs, device-pairing (v1.500.14); paperless-settings, notification-bell (v1.500.15); eslint no-undef-Netz + `npm run lint` + Fix `cmModule`-Live-Bug im Files-Editor (v1.500.16).
- **Nächste Scheiben:** Stores (confirm/nav/vault/paperless), einfache Komponenten (toastHub/cropModal/…), dann gallery/files/passwords/contacts als eigene Komponenten-Dateien, dann Stores + einfache Komponenten, zuletzt gallery/files/passwords in Mixins.
